...
**[Extract tests from specials module]**
**Tangle:** The `doom-game/src/specials.rs` file had grown to nearly 10,000 lines, heavily padded with inline tests, causing "The Blob" anti-pattern and making the core logic difficult to navigate.
**Blueprint:** Refactored `specials.rs` into a directory module (`specials/mod.rs` and `specials/tests.rs`), extracting the tests to improve navigability without altering the external API.
