...
**[Remove unused doom-game dependency from doom-demo]**
**Tangle:** The `doom-demo` crate had a direct dependency on the `doom-game` crate in its `Cargo.toml`. This violated the separation of concerns since demo playback logic shouldn't inherently depend on the entirety of the game simulation logic, but rather just on the shared `doom-types` data format to process commands. `doom-demo` was actually only referencing `doom-game` via doc comments.
**Blueprint:** Removed the `doom-game` dependency from `crates/doom-demo/Cargo.toml`. This breaks the cycle and reinforces that `doom-demo` only requires core types, allowing it to build without needing the full game state machinery.
