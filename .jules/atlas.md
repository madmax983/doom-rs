...
**[Remove Leaky limits from doom-game/src/player.rs]**
**Tangle:** `crates/doom-game/src/player.rs` had a `pub use doom_types::limits::{NUM_POWERS, NUM_PSPRITES};` which leaked those limits out to any users of the `doom-game` crate. This violates boundary isolation since `doom-types` limits should be imported directly by whoever needs them from `doom-types`, not through `doom-game`.
**Blueprint:** Removed `pub use doom_types::limits::{NUM_POWERS, NUM_PSPRITES};` from `player.rs` and replaced the other internal usages by adding `NUM_POWERS` and `NUM_PSPRITES` directly to the `use doom_types::limits::...` statement at the top of the file so `player.rs` still compiles correctly without exporting them.
