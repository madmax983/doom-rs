...
**Tangle:** The `ExitRequest` and `LockedDoorColor` enums were defined in `doom-game/src/state.rs`. However, they are used by presentation crates like `doom-app` (for rendering HUD messages on locked doors) and other game logic routines. This caused UI components to depend directly on the entire game engine crate's `state` module just for basic enums, creating tight coupling between rendering and core game state, violating domain boundaries.
**Blueprint:** Extracted `ExitRequest` and `LockedDoorColor` into `doom-types/src/primitives.rs`. Updated `doom-game` and `doom-app` imports to reference these primitives from the shared types crate, creating a clean dependency hierarchy where both presentation and logic rely on foundational types.
