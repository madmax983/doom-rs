...
**Tangle:** `ExitRequest` and `LockedDoorColor` enums were defined in `doom-game`, but widely used in presentation crates like `doom-app`. This caused UI components to depend directly on the entire game engine crate just for basic enum definitions, creating tight coupling between rendering and core game state.
