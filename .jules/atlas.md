...
**[Enforce Facade pattern in workspace crates]**
**Tangle:** Several workspace crates (`doom-net`, `doom-demo`, `doom-audio`, `doom-wad`) were exposing their internal implementation details via `pub mod` at the crate root. This created leaky abstractions, allowing upstream crates to bypass the public API and depend on internal structures.
**Blueprint:** Modified `pub mod` to `mod` in the `src/lib.rs` files for these crates. Fixed missing re-exports of required types (e.g., `encode_pcm16_wav_mono`, `SfxPriority`) and updated downstream callers in `doom-app` and `doom-renderer` to import these types from the root namespace rather than reaching deep into the module hierarchy. Added `#[allow(dead_code)]` to unused items flagged by Clippy after the visibility change, carefully avoiding separation of `#[derive(...)]` macros from struct definitions. This enforces strict module boundaries and forces downstream crates to respect the public API contract.
