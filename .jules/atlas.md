...
**[Enforce module boundaries in doom-demo and doom-net]**
**Tangle:** The `doom-demo` and `doom-net` crates contained several internal modules marked as `pub mod`, leaking their contents instead of enforcing boundaries through `pub use` statements at the crate root. `doom-net` also exposed `make_join_packet` and `is_join_response` unnecessarily, resulting in dead-code warnings when hidden. Intra-doc links in `doom-demo` became broken when modules were made private.
**Blueprint:** Altered the visibility of internal modules to `pub(crate) mod` in `crates/doom-demo/src/lib.rs` and `crates/doom-net/src/lib.rs`. Maintained explicit re-exports using `pub use`. Added `#[allow(dead_code)]` to unused handshake functions in `doom-net`, and replaced `[`link`]` intra-doc syntax with `` `code` `` backticks in `doom-demo` for private modules. This strictly enforces clean API surfaces.
