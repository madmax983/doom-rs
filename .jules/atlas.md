...
**[Stop GameState Re-export Leak]**
**Tangle:** `doom-game/src/state.rs` was unnecessarily re-exporting `doom_game::movers::*` and `doom_game::sound_prop::*` components into the `state` module using `pub use`. This caused other modules inside `doom-game` to depend on `state::` for components (like `SectorMovers`, `FloorType`, `SoundRequest`) that rightfully belonged in their own submodules (`movers`, `sound_prop`). This broke strict domain boundaries, as `state` should be the top-level aggregator, not a pass-through import namespace.
**Blueprint:** Removed the `pub use` re-exports from `doom-game/src/state.rs`, turning them into internal module imports. Updated all internal module dependents (like `specials.rs`, `savegame.rs`, `weapon_fire.rs`) to fetch these structs and enums directly from `movers::` and `sound_prop::`. This enforces cleaner dependency arrows where internal submodules fetch the specific domain types directly.
