...
**[Extract DoomGame orchestrator from main.rs]**
**Tangle:** The `doom-app/src/main.rs` file was an enormous "God File" containing over 5,200 lines, largely due to the inline definition and implementation of the `DoomGame` struct (and associated enums like `WeaponMotion` and `PlayerStateCarry`). This created a classic "Blob" anti-pattern, mixing application startup, networking, demo logic, and the core game loop orchestrator in a single file.
**Blueprint:** Extracted the `DoomGame` struct, `WeaponMotion`, `PlayerStateCarry`, and all of their `impl` blocks into a dedicated `crates/doom-app/src/game.rs` module. Updated `main.rs` to declare `pub(crate) mod game;` and re-export these types. This significantly improves file cohesion, navigation, and boundary isolation within the `doom-app` orchestrator.
