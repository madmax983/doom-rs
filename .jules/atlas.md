...
**[Extract Boss Brain State to boss_brain.rs]**
**Tangle:** The `doom-game/src/state.rs` file contained `GameState`, which had grown into a "God Struct" holding fields directly related to the Boss Brain (Icon of Sin): `brain_awake`, `brain_targets`, and `brain_target_index`. This violated domain boundaries and reduced cohesion by placing highly specific entity state directly into the global simulation state.
**Blueprint:** Created a new module `boss_brain.rs` containing a `BossBrainState` struct. Extracted the `awake`, `targets`, and `target_index` fields from `GameState` into `BossBrainState` and updated `actions.rs` to reference `gs.boss_brain`. Also updated `spawn_level_things` to populate `boss_brain.targets` directly when encountering DoomEd thing type 87 (Spawn spots).
