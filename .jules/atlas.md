...
**[Stop limits Re-export Leak]**
**Tangle:** `doom-game/src/player.rs` and `doom-game/src/lib.rs` were unnecessarily re-exporting `doom_types::limits::{NUM_POWERS, NUM_PSPRITES}` via `pub use`. This caused presentation crates like `doom-app` to potentially depend on the game engine `doom-game` just to use basic shared limit data types. This violated the boundary isolation.
**Blueprint:** Removed the re-exports from `doom-game` and updated `doom-game` to import `NUM_POWERS` and `NUM_PSPRITES` directly from the foundational `doom-types` crate. This enforces cleaner dependency arrows where higher-level crates fetch domain primitives directly from the shared types crate rather than pulling them through the game logic.
