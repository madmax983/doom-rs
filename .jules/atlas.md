...
**[Extract specials tests to separate file]
**Tangle:** `crates/doom-game/src/specials.rs` was a huge file (nearly 10,000 lines long), making it hard to navigate. Much of this bloat was due to extensive inline tests (over 6,000 lines).
**Blueprint:** Split `specials.rs` into `specials/mod.rs` and `specials/tests.rs` to keep the module structure while greatly reducing the primary file size and separating concerns, reducing the "Blob" anti-pattern.
