...
**[Re-export Leaks]
**Tangle:** `doom-game/src/state.rs` and `doom-game/src/player.rs` unnecessarily re-exported internal modules `sound_prop`, `movers`, and limits via `pub use`, exposing implementation details and breaking boundaries.
**Blueprint:** Altered the visibility of the re-exports from `pub` to `pub(crate)` in `state.rs` and `player.rs`. Updated dependent code in `specials.rs` to import `CeilingType` directly from `movers.rs` instead of relying on the re-export. Enforced strict internal encapsulation.
