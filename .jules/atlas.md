...
**[Encapsulate SectorGraph adjacency list]**
**Tangle:** The `SectorGraph` struct in `doom-map` exposed its internal `adjacency_list: HashMap<usize, HashSet<usize>>` as `pub`. This created a leaky abstraction that exposed the specific memory layout and implementation details to downstream crates, allowing potential violation of graph invariants.
**Blueprint:** Modified `pub adjacency_list` to `pub(crate) adjacency_list` to enforce encapsulation, ensuring external crates cannot rely on or mutate the internal topological graph structure, maintaining clear module boundaries. Used `pub fn from_adjacency_list_for_testing` to resolve integration test dependencies securely without exposing data to the public API.
