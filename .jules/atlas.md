...
**[Extract keys and powers constants to doom-types]**
**Tangle:** The `KEY_*` and `PW_*` (powers) constants were defined in `doom-game/src/player.rs`, creating implicit, tight coupling between the core game engine crate and presentation crates like `doom-app` and `doom-renderer` that relied on them to display HUD indicators or power-ups.
**Blueprint:** Extracted the key and power-up constants into `doom-types/src/keys.rs` and `doom-types/src/powers.rs`. Updated callers in `doom-game`, `doom-app`, and `doom-renderer` to consume these foundational primitives directly from `doom-types`, rather than coupling to the `player` state module. This reinforces the separation of concerns between primitives and game logic.
