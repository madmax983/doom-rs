...
**[Extract Keys and Powers to Shared Primitives]**
**Tangle:** The `KEY_*` constants and `powers` module were defined in `doom-game/src/player.rs` and leaked publicly. This caused presentation and utility crates like `doom-app` and `doom-renderer` to depend directly on the game engine logic crate just to use basic state indices and key limits, violating boundary isolation and creating tight coupling between rendering and core game logic.
**Blueprint:** Extracted the `KEY_*` constants into `doom-types/src/keys.rs` and the `powers` module into `doom-types/src/powers.rs`. Updated `doom-game/src/player.rs` to import these shared primitives using `pub(crate) use` to prevent them from leaking again. Modified `doom-app` and `doom-renderer` to fetch these primitives directly from the foundational `doom-types` crate, enforcing a cleaner dependency graph.
