...
**[Stop NUM_POWERS Re-export Leak]**
**Tangle:** `doom-game/src/player.rs` was unnecessarily re-exporting `doom_types::limits::{NUM_POWERS, NUM_PSPRITES}` via `pub use`. This caused potential leaks and violated the boundary isolation by offering types that already reside in a primitives crate through a higher level crate.
**Blueprint:** Removed the re-export from `doom-game/src/player.rs` by replacing `pub use` with `use`.
