...
**Re-export Leak in state.rs**
**Tangle:** `doom-game::state` unnecessarily re-exported internal types (`SoundPropagation`, `SoundRequest`, and `movers::*`) using `pub use`, creating a leaky abstraction and polluting the public API since these were already exported at the crate root.
**Blueprint:** Changed the visibility modifiers in `state.rs` to `pub(crate) use` to restrict access strictly to the crate internals and enforce clear module boundaries.
