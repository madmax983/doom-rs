...
**[Stop state.rs Re-export Leak]**
**Tangle:** The `doom-game/src/state.rs` module was re-exporting all of `movers::*` and `sound_prop::*` via `pub use`, leaking implementation details and causing tight coupling where modules rely on `crate::state::` for types that actually belong to `movers` or `sound_prop`.
**Blueprint:** Removed the `pub use` statements from `state.rs` and updated internal references across `doom-game` to import types directly from `crate::movers` and `crate::sound_prop`. This enforces strict encapsulation and correct dependency graphs.
