...
**[Extract StyleMeter to Shared Primitives]**
**Tangle:** The `StyleMeter` and `StyleRank` system (along with the `style_meter` feature flag) were defined in `doom-game/src/style.rs`, but heavily used in presentation crates like `doom-tui` and `doom-app` for rendering cogmind mode and the style rank HUD. This caused UI components to depend directly on the entire game engine crate just for basic scoring structures and enum definitions, creating tight coupling between rendering and core game state, leading to a leaky abstraction.
**Blueprint:** Extracted the entire `style.rs` module from `doom-game` into `doom-types/src/style.rs` to keep pure logical primitives in the foundational crate. `doom-game/src/style.rs` now simply re-exports `StyleMeter` and `StyleRank` for backward compatibility within the game logic, and UI components in `doom-tui` and `doom-app` can access these primitives via `doom_types` without needing the whole `doom_game` crate to compile correctly with the `style_meter` feature. The `doom_game::style::StyleRank` calls in `doom-tui` were rewritten to use `doom_types::style::StyleRank` instead. This enforces cleaner dependency boundaries where presentation layer components fetch structs directly from the shared types crate.
