...
**[Fix TUI deprecated skip method]**
**Tangle:** The `doom-tui` crate used `ratatui::buffer::Cell::set_skip` in its event_loop and sixel drawing which was deprecated and causing a `cargo clippy` error when compiling with `-D warnings`. Using the ratatui recommended solution of `set_diff_option(ratatui::buffer::CellDiffOption::Skip)` does not exist and fails to compile.
**Blueprint:** Kept the original `set_skip` method but wrapped it with `#[allow(deprecated)]` to explicitly suppress the deprecation warning without breaking the build logic.
