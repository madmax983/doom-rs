...
**[Extract SectorDamageType]**
**Tangle:** `SectorDamageType` was placed in `movers.rs` despite only being used in `specials.rs` for applying periodic sector damage, causing an unnecessary cross-module boundary for an internal concept.
**Blueprint:** Moved `SectorDamageType` into `specials.rs` and removed the `pub` visibility modifier. Updated all occurrences inside `specials.rs` to no longer reference it through `crate::state::`. This strictly encapsulates the type within the only module that uses it.
