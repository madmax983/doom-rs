...
**[Extract powers and psprite_slots to Shared Primitives]**
**Tangle:** The `powers` and `psprite_slots` sub-modules were defined in `doom-game/src/player.rs` and unnecessarily re-exported. This caused UI components to depend directly on the game engine logic just for basic slot indices.
**Blueprint:** Extracted `powers` and `psprite_slots` into `doom-types/src/powers.rs` as independent constants. Updated imports in `doom-app` and `doom-game` to import these primitives directly from the shared types crate, creating a clean dependency hierarchy where presentation relies on foundational types.
