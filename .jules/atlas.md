...
**[Eliminate Boolean Blindness in DoorMover]**
**Tangle:** The `DoorMover` struct in `doom-game` used an `is_ceiling: bool` field to determine if the mover was operating on a ceiling or a floor. This "Boolean Blindness" anti-pattern obscured the intent at call sites and made it easier to introduce bugs where a boolean is misread or passed incorrectly.
**Blueprint:** Replaced the primitive boolean with a strictly typed `MoverTarget` enum containing `Ceiling` and `Floor` variants. Updated serialization logic in `savegame.rs` to persist the value as a boolean to maintain backward compatibility of the save game binary format. Updated `linedef_dispatch.rs` and `specials.rs` to use the new typed enum to ensure explicit intent and correctness.
