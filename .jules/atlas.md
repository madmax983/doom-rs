...
**[Extract tests from massive specials.rs]**
**Tangle:** The `crates/doom-game/src/specials.rs` module was nearly 10,000 lines long, with more than 6,000 lines just being tests. This is a severe "Blob" anti-pattern that makes the file unreadable and triggers editor performance issues.
**Blueprint:** Extracted the massive `mod tests` block into a dedicated `crates/doom-game/src/specials/tests.rs` file. This is the safest and most effective first step to reduce file size without touching highly-coupled internal implementation logic or triggering massive compilation cascades.
