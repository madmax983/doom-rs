...
**[Deduplicate cheat implementations]**
**Tangle:** The `doom-app` crate defined its own `CheatDetector` and `apply_cheat` logic in `cheats.rs` which duplicated the identical string/byte sequence logic already present in `doom-game::cheats`. It also handled the `IDDT` (full map reveal) cheat as a purely app-level string match.
**Blueprint:** Removed `doom-app/src/cheats.rs` entirely. Refactored the console logic to utilize the shared `CheatBuffer` and `apply_cheat` methods from `doom-game`, converting the typed console strings into byte buffers for the game engine to match. Moved the `IDDT` cheat down into `doom-game::cheats::CheatCode::FullMapReveal` to keep all code matching unified, while leaving the side-effect (toggling the UI overlay) up in the app layer.
