...
**Tangle:** `doom-renderer/src/lib.rs` was unnecessarily re-exporting `doom_game::AutomapState` via `pub use`. This caused presentation crates like `doom-app` to depend on the renderer crate just to use basic shared state types from the game logic. This violated the boundary isolation.
**Blueprint:** Removed the re-export from `doom-renderer` and updated `doom-app` to import `AutomapState` directly from the foundational `doom-game` crate. This enforces cleaner dependency arrows where higher-level crates fetch domain state directly from the source logic crate rather than pulling them through the renderer.
