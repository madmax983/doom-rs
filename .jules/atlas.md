...
**[Remove doom-game dependency from doom-demo]**
**Tangle:** The `doom-demo` crate had a dependency on the `doom-game` crate in its `Cargo.toml`. However, the code only actually uses foundational `doom_types::TicCmd` and `doom_types::bt` from the `doom-types` crate. This unnecessary dependency introduced a layer of coupling where a generic demonstration playback crate incorrectly depended on the whole game engine.
**Blueprint:** Removed the `doom-game` dependency from `crates/doom-demo/Cargo.toml`. Since `doom-demo` already correctly imported what it needed from `doom-types`, no Rust code changes were required. This enforces a cleaner, acyclic graph where utility crates do not depend on massive game logic crates unnecessarily.
