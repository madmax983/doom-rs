...
**[Extract specials tests]**
**Tangle:** The `crates/doom-game/src/specials.rs` module had grown too large (The "Blob" anti-pattern), comprising nearly 10,000 lines, where more than 6,000 lines were just tests inside `#[cfg(test)] mod tests { ... }`.
**Blueprint:** Extracted the massive `tests` module into a separate file `crates/doom-game/src/specials/tests.rs` using `#[cfg(test)] mod tests;` inside `specials.rs`. This significantly reduces file size, reduces compile-time memory footprint for that single file, and improves readability without breaking the module graph.
