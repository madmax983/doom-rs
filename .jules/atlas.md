...
**Tangle:** `doom-game/src/player.rs` was unnecessarily re-exporting `doom_types::limits::{NUM_POWERS, NUM_PSPRITES}` via `pub use`. This causes presentation crates to depend on the game engine `doom-game` just to use basic shared limits data types, violating boundary isolation.
**Blueprint:** Altered the re-export to `pub(crate) use` to enforce strict boundaries and prevent future leakage. This ensures strict internal encapsulation.
