...
**[Extract Powers and PSpriteSlots to Shared Primitives]**
**Tangle:** The `powers` and `psprite_slots` constants modules were defined inside `crates/doom-game/src/player.rs` and marked as `pub mod`. This caused presentation modules like `doom-app/src/cheats.rs` and `doom-app/src/main.rs` to reach deep into the `doom-game` game logic crate just to fetch basic constants indices like `PW_INVULNERABILITY`. This created strong coupling, violating domain boundaries.
**Blueprint:** Extracted the `powers` and `psprite_slots` constant definitions into their own files `crates/doom-types/src/powers.rs` and `crates/doom-types/src/psprite_slots.rs`. `crates/doom-game/src/player.rs` now re-exports them for backwards compatibility within the game logic crate, and `doom-app` was updated to import these primitives directly from the shared `doom-types` crate. This enforces a cleaner dependency hierarchy where high-level presentation logic depends only on foundational shared types rather than game simulation details.
