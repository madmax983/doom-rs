...
**Tangle:** `doom-game/src/player.rs` was unnecessarily re-exporting `doom_types::limits::{NUM_POWERS, NUM_PSPRITES}` via `pub use`. This caused presentation crates and other modules to depend on the game engine `doom-game` just to use basic shared limits. This violated the boundary isolation.
**Blueprint:** Removed the re-exports from `doom-game` and updated `doom-game/src/player.rs` to just `use` them instead of `pub use`. This enforces cleaner dependency arrows where higher-level crates fetch domain limits directly from the shared types crate rather than pulling them through the game logic.
