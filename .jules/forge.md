...
**[God Function refactor via struct bundling in DFS]
**Learning:** Functions implementing complex algorithms (like iterative DFS for Tarjan's bridge-finding) often accumulate a large number of internal state variables (`visited`, `discovery_time`, `low_time`, `parent`, `time`), leading to a "God Function" smell and violating readability.
**Action:** Bundle these related local state variables into a dedicated, `Default`-derivable internal struct (e.g., `DfsState`) and extract the inner loop into a separate helper function taking `&mut DfsState` to drastically flatten nesting and reduce cognitive load.
