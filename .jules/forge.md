...
**Refactored run_doom God Function**
**Learning:** `run_doom` grew over 800 lines by heavily embedding specialized CLI argument execution logic (like map statistics printing, pathfinding, and tactical analysis graph evaluation).
**Action:** Extract large blocks of specialized evaluation logic that operate sequentially on the data into separate helper functions to flatten the code and reduce cognitive load on the orchestrator function.
