...
**Learning:** `tick()` in `crates/doom-app/src/main.rs` is almost 300 lines long and handles many unrelated things (menu, console, title screen, cheat codes, sound events, player damage, etc.). This is a classic "God Function".
**Action:** Extract logical blocks from `tick()` into smaller helper functions.
