...
## 2026-07-26 - Refactored complex conditionals and fixed precedence in doom-game
**Learning:** Obfuscated `if/then` chaining and single `match` statements increase cognitive overhead, and operator precedence around bitwise shifts can cause subtle bugs. Combining identical condition branches reduces boilerplate.
**Action:** Replace `match` for single variants with `if let`, merge identical `if` blocks with logical ORs, explicitly parenthesize bitwise shifts combined with arithmetic, and use `contains` for range checking.
