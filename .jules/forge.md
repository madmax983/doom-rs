...
**Clippy Refactoring**
**Learning:** Overly complex functional chains and match statements can obscure intent and trigger clippy warnings.
**Action:** Apply clippy suggestions like using `if let` for single match arms, `if else` instead of boolean mapping, and combining identical branches to improve readability.
