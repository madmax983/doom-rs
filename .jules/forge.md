...
**Learning:** `FaceState::tick` taking multiple booleans like `is_firing` and `is_invulnerable` creates "Boolean Blindness", obscuring intent at the call site.
**Action:** Replace `bool` with descriptive enums like `IsFiring` and `IsInvulnerable` to strongly type API boundaries and self-document the code at the call site.
