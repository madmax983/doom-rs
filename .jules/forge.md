...
**Flatten monolithic functions using helper methods**
**Learning:** Functions like `DoomGame::tick` can become "God Functions" (over 200 lines long) with deep nesting and multiple responsibilities, making them difficult to read and maintain.
**Action:** Extract logical blocks (like handling the title screen, intermission, console/cheats, save/load, and player damage) into private helper methods. Use early returns in the main function to avoid deep nesting and improve readability, ensuring zero behavior change.
