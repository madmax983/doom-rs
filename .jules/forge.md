...
**Learning:** Functions like `next_map` taking a boolean `secret_exit` parameter obscures intent at the call site and causes Boolean Blindness (e.g. `next_map(true)`). In many cases, there is already an existing enum that represents the concept (e.g., `ExitRequest`) that can be reused.
**Action:** Replace `secret_exit: bool` with `exit_request: ExitRequest` and refactor the internal logic to use `exit_request == ExitRequest::Secret` to improve call site readability and strongly type the API boundary.
