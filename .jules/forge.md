...
**[Flatten Option extraction in analyzer.rs and savegame.rs]
**Learning:** Pyramids of doom from nested `if let` and sequential block reading obscure logic.
**Action:** Use `if let` guard clauses to safely flatten deep nesting and extract repetitive tuple Option unwrapping or sequence reading into helper blocks.
