...
**[Refactoring manual state flags in double loops]**
**Learning:** Using a mutable boolean flag like `skip_first` to skip the first iteration of a nested double `for` loop causes visual noise and "boolean blindness", qualifying as a recurring anti-pattern.
**Action:** Replace mutable skip flags with explicit coordinate matching (e.g., `if x == area.left() && y == area.top() { continue; }`) to make the skipping logic stateless and clearer without resorting to code golf iterator chains.
