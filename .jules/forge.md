...
**Refactored Boolean Blindness in weapon_fire**
**Learning:** Using `bool` arguments in functions like `hitscan_shot_angle(gs, autoaim_angle, true)` suffers from boolean blindness, making the intent of `true` unclear at the call site.
**Action:** Extracted the boolean parameter into a strongly typed `HitscanAccuracy` enum (`AccurateFirstShot`, `AlwaysSpread`) to self-document the code at the call site and enforce valid states.
