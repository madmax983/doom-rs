...
## 2024-06-04 - Refactored Option destructuring boolean blindness in actions.rs
**Learning:** Using `.map(|mo| condition).unwrap_or(false)` to access fields from a collection like `gs.mobjslab.get(handle)` creates verbose and disconnected functional chains that decrease readability.
**Action:** Use idiomatic `.map_or(default, |mo| ...)` or `.is_some_and(|mo| ...)` to safely evaluate inner conditions while preserving read flow and minimizing boilerplate.
