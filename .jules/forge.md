...
**[Refactored Boolean Blindness in next_map]**
**Learning:** `MapId::next_map` taking a boolean `secret_exit` causes Boolean Blindness, obscuring intent at the call site.
**Action:** Replaced `bool` with an `ExitType` enum (`Normal` vs `Secret`) to strongly type API boundaries and self-document the code at the call site.
