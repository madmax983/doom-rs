...
**Refactoring Boolean Blindness in `next_map` and `hitscan_shot_angle`**
**Learning:** Functions like `next_map(&self, secret_exit: bool)` and `hitscan_shot_angle(..., accurate_first_shot: bool)` suffer from Boolean Blindness, hiding intent at the call site.
**Action:** Replace `bool` with descriptive enums or existing structs like `ExitRequest` and `ShotAccuracy` to strongly type API boundaries and self-document the code at the call site.
