...
**Refactoring nested guard clauses over multiple sidedef lines**
**Learning:** The idiom `let Some(sd) = level.sidedefs.get(x) else { return false; }; let sector_idx = sd.sector as usize;` followed by `if sector_idx >= ... { return false; }` introduces unnecessary intermediate variables and is needlessly verbose.
**Action:** Use `.map(|sd| sd.sector as usize)` directly in the guard condition to inline the lookup: `let Some(sector_idx) = level.sidedefs.get(x).map(|sd| sd.sector as usize) else { return false; };`.