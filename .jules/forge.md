...
**Refactored target floor calculation in ev_do_donut**
**Learning:** Manual traversal of linedefs and sidedefs to find adjacent sectors introduces boilerplate, logic duplication, and deeply nested loops, reducing readability.
**Action:** Replace manual linedef iterations for adjacent sector discovery with the existing `adjacent_sectors` iterator pipeline, making the logic flatter and strictly tied to existing domain abstractions.
