...
**Refactored LightEffectType logic**
**Learning:** Repetitive inline `match` blocks configuring enum-based state create noisy and duplicated boilerplate inside processing loops.
**Action:** Extract the `match` blocks into methods on the enum itself to encapsulate the configuration details and clean up the call sites.
