...
**[Boolean Blindness in sync_modifiers]**
**Learning:** `InputState::sync_modifiers` taking `shift: Option<bool>, control: Option<bool>` creates Boolean Blindness at the call sites, making them confusing (e.g. `s.sync_modifiers(None, Some(true))`).
**Action:** Group these modifier flags into a strongly typed `ModifierSnapshot` struct to self-document intent at the call site.
