...
**Refactor manual for loops into iterator chains to find items**
**Learning:** Finding a single item using a `for` loop with `break` introduces a mutable `Option` and increases cognitive load.
**Action:** Prefer `into_iter().find_map(...)` over `for` loops when conditionally searching for an item in a sequence.
