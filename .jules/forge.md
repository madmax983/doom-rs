...
**[Boolean Blindness in Mover States]**
**Learning:** Ambiguous booleans like `is_ceiling: bool` (in `DoorMover`), `is_bright: bool` (in `LightSpecial`), and `waiting: bool` (in `FloorMover`) obscure intent and make state management bug-prone. Wait, `FloorMover` also had a disjointed `wait_remaining` field alongside `waiting: bool`.
**Action:** Extract primitive clusters into strong descriptive enums (e.g., `MoverTarget::Ceiling`, `LightPhase::Bright`, and `FloorMoverPhase::Waiting(i32)`) to strictly enforce valid data invariants and improve call-site clarity.
