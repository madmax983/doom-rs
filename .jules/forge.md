...
**[Refactored Boolean Blindness in ev_build_stairs]**
**Learning:** `ev_build_stairs` taking multiple primitive configuration parameters (`stair_type: StairType`, `crush: CrushBehavior`) alongside core contextual arguments creates long parameter lists and obscures intent.
**Action:** Group these configuration parameters into a named struct like `StairParams` to self-document call sites, reduce argument counts, and improve API readability.
