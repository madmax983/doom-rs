...
## 2024-05-19 - Refactored Boolean Blindness in next_map
**Learning:** Functions like `next_map(secret_exit: bool)` (and its internal helpers) suffer from Boolean Blindness, hiding the true intent at the call site (`next_map(true)` vs `next_map(false)`).
**Action:** Replaced `bool` with a strongly typed enum `ExitType` (`Normal`, `Secret`) to document intent and enforce correct usage at compile time across the codebase and tests.
