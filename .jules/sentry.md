...
## 2025-05-10 - Added Missing `StateNum::NULL` test for tick_psprite_slot fallback

**Learning:** Found an uncovered branch related to the fallback `StateNum::NULL` handling in `weapons.rs` when `unwrap_or(StateNum::NULL)` defaults due to the current state being out of bounds of the STATES array.
**Action:** Added targeted test case `tick_psprite_with_invalid_state_sets_null` in `weapons.rs` to reach 100% test coverage on state transitions.
