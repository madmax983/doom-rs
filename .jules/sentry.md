...
## 2026-05-18 - Replacing unwrap with expect in doom-game tests
**Learning:** Found scattered instances of `.unwrap()` in test modules within `doom-game` (`specials.rs`, `tic.rs`, and `dehacked.rs`), which obscure test failure context by panicking with generic messages. This violates the testing philosophy that tests should provide meaningful failure context.
**Action:** Always prefer `.expect()` over `.unwrap()` when extracting values from Options or Results in test code to explicitly document invariants and provide clear context during failures.
