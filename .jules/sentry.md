...
## 2024-05-18 - Remove unwrap() in tests and modules
**Learning:** The use of `unwrap()` in test files and internal logic obscures test failure context by panicking with a generic error message.
**Action:** Always replace `unwrap()` with `expect()` in tests and other files to provide meaningful context and explicitly document the invariant.
