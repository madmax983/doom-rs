...
## 2026-07-24 - Testing Parsing Boundaries for thing_flags in doom-map

**Learning:** `thing_flags` in the `udmf.rs` parser maps multiple individual semantic fields (e.g., `skill1`, `single`) to a legacy 16-bit flags integer. Many of these branches were previously untested, risking bugs when converting UDMF constructs into core engine structs. Testing these branches directly using a table-driven approach targeting the parser avoids needing to mock full `Level` structures.
**Action:** Use table-driven tests (structs/tuples) for deterministic parsing logic like `thing_flags`. This directly tests the logic in isolation rather than relying on full game integration tests.
