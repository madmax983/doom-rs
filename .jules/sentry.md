...
## 2026-07-12 - Ensure full edge-case coverage on parsers
**Learning:** Error formatting (`thiserror` Display) and Option fallbacks (e.g. `unwrap_or("")` on failed UTF-8 decode) often escape coverage tools due to being implicit or auto-generated, but are critical for operator panic safety.
**Action:** Always write explicit tests asserting the exact `to_string()` output of library error enums and specific fallback behavior of string formatting routines in low-level parsers.
