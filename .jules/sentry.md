...
## 2024-05-18 - Replacing unwrap() in tests
**Learning:** `unwrap()` calls in tests obscure test failure context by panic-ing with a generic error message, violating Sentry's principle that tests should provide meaningful context.
**Action:** Always replace `unwrap()` with `expect("descriptive message")` in tests to explicitly document the invariants in tests across multiple modules to assist with debugging.
