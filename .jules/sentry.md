...
## 2026-07-17 - Table-Driven / Full Branch Testing in Lexing
**Learning:** For hand-written lexers (like the UDMF parser), every single branching pathway (such as exponent parsing, escape sequence matching) and every bitwise flag assignment must have explicit coverage to ensure edge cases (e.g. malformed exponents, specific comment terminations) are caught, preventing silent mis-parsing or panics in production.
**Action:** Always include table-driven or comprehensive multi-field test cases that exercise every possible output branch, especially for flag mapping and string/number lexing.
