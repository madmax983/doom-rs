...
## 2026-07-25 - Fix clippy warnings
**Learning:** We had many clippy warnings from unused bindings and logical inconsistencies, like redundant checks in `tic.rs` and `combat.rs`. We had to refactor them for better control-flow structure and correct evaluation without ignoring side-effects.
**Action:** Always make sure `clippy` finishes without warnings by verifying the codebase frequently to catch potential issues early.
