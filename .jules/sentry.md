...
## 2024-05-24 - Missing edge case testing in SpriteClipHistory\n\n**Learning:** The manual ArrayVec-like structure `SpriteClipHistory` in `doom-renderer/src/sprite_clip.rs` was almost entirely lacking test coverage, particularly around capacity limit enforcement and boundary conditions for pushing multiple elements to a stack.\n**Action:** Wrote `push_beyond_capacity_drops_extra`, `default_and_new_are_empty`, `push_within_capacity_updates_len_and_last` and `iter_returns_pushed_items_in_order` tests to achieve 100% test coverage for sprite clipping stack logic.
