...
## 2024-05-18 - Missing edge case testing in UDMF Map Parsing
**Learning:** The UDMF textmap parser in `udmf.rs` lacked tests verifying its behavior on various edge cases and boundary conditions. This included handling malformed identifiers (`123bad`), EOF states during string iteration, unterminated block comments, and parsing of floats out-of-range for expected 16-bit integrals (`1e30`). Additionally, there was limited test coverage over flag bitwise composition operations in `linedef_flags` and `thing_flags` which process the boolean fields.
**Action:** Wrote 12 new comprehensive unit tests ensuring failure states properly map to explicit `UdmfError` enum variants to prevent parser panic states. Added tests covering the specific bitwise parsing output of `linedef` and `thing` blocks to ensure accurate AST conversion paths.
