...
## 2026-05-18 - Missing edge case testing for empty BSP Trees
**Learning:** Functions in `BspTree` (`point_in_subsector` and `max_depth`) contained edge-case conditions handling empty tree instances (no nodes, only 1 subsector). These branches were untested and presented a blind spot. Also discovered numerous `expect("value must exist in test")` and `unwrap_err()` in `udmf.rs` and `bsp.rs` which obscured error context.
**Action:** Wrote tests `point_in_subsector_empty_tree_returns_only_subsector` and `max_depth_empty_tree_returns_zero` to achieve 100% test coverage of BSP edge cases. Replaced generic expects with descriptive messages to adhere to Sentry guidelines.
