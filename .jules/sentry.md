...
## 2024-07-14 - udmf.rs Parsing edge cases
**Learning:** Found several uncovered parsing paths in UDMF like missing `type` identifiers falling back incorrectly, float formatting strings, boundary checks for `i64` overflow, block comments, and string escapes causing crashes on invalid UDMF lump data.
**Action:** Added extensive tests targeting extraction algorithms (`integral_value`, `first_present_u16`, `optional_sidedef_index`) to guarantee predictable rejection behaviors.
