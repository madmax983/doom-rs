...
## 2026-08-10 - Missing invalid state transition to StateNum::NULL test for tick_psprite_slot
**Learning:** Discovered that while `tick_mobj` was tested for `StateNum::NULL` fallbacks, the equivalent player weapon state logic in `tick_psprite_slot` was untested when encountering invalid next states.
**Action:** Add targeted unit tests when `unwrap_or(StateNum::NULL)` is used as a fallback for invalid state transitions.
