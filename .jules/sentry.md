...
## 2024-07-28 - UDMF Parser Edge Cases
**Learning:** Testing complex parsers (like UDMF) requires rigorous checks of unhandled token starts and bounds check fallbacks. Even unused fields like `unknown` blocks must be explicitly verified to be ignored without panic.
**Action:** When auditing custom AST parsers, ensure string escape sequence loops and numeric overflow edge cases are explicitly tested before assuming `unwrap()` won't trigger.
