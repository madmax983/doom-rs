...
## 2024-05-25 - Increased UDMF parser error coverage
**Learning:** The UDMF AST parser returns many semantic errors at compilation time (`into_level_data`) rather than parse time. Adding a table-driven test module successfully exercised these missing branches without having to mock the `Level` struct or `doom-game` side logic.
**Action:** Always prefer table-driven testing with tuples or structs for deterministic matching logic, like validating complex combinations of bits to determine the correct entity representation from a parsed struct (e.g. `thing_flags`).
