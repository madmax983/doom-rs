...
## 2026-08-02 - MapAnalyzer resilience against malformed topologies

**Learning:** `MapAnalyzer::chokepoints` previously panicked using `.unwrap()` if the sector graph had disconnected or asymmetric edges where a neighbor referenced a missing node in the adjacency list.
**Action:** Replaced `unwrap()` with `unwrap_or(&HashSet::new())` to provide empty iterators for missing values safely, and added a specific test case `havoc_test_analyzer_does_not_panic_on_asymmetric_edges` to prevent panics gracefully.
