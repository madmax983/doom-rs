...
## 2026-07-13 - Testing UDMF Parser Gaps

**Learning:** `cargo-llvm-cov` is more reliable than `cargo-tarpaulin` and provides accurate HTML line coverage. When a simple gap in `doom-types` involved parallel test execution mutating a global atomic `FINESINE_READY` state, it became difficult to reliably assert the pre-initialization branches. Switching focus to `doom-map` revealed significant parsing logic gaps in `udmf.rs` (block comments, exponent errors, unexpected tokens).

**Action:** If a trivial `unsafe` or global state test requires thread serialization tricks, re-prioritize to complex data parsing components (like `udmf.rs`) where integration tests provide significant, deterministic coverage value without environment interference.
