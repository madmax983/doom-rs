...
## 2024-05-25 - Replace generic unwrap calls with expect in doom-map analyzer and lumps

**Learning:** Found `.unwrap()` calls without meaningful context in `MapAnalyzer` (which assumes nodes exist since it loops over keys) and lump parsing logic (`try_into()` on slices of guaranteed static size). Using `unwrap()` obscured whether an internal logic error or malformed input data caused the panic.
**Action:** Always replace `.unwrap()` with `.expect("description of invariant")` when the value is mathematically or logically guaranteed to exist to improve debuggability.
