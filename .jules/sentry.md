...
## 2026-05-18 - Ensure enums map bounds correctly with targeted max boundary tests
**Learning:** `GameMode::max_episode` lacked a unit test verifying each enum variant matched its specific episode cap correctly. Simple mapping functions are prone to missed edge cases.
**Action:** Always add tests to ensure simple match arms mapping states to scalar bounds return exact expected values, ensuring correct boundaries.
