...
## 2026-07-21 - InputState Edge Case Coverage
**Learning:** Found multiple untested paths in `InputState` relating to UI triggers (menu navigation, escape) and alternative modifier keys (RightShift, RightControl). While basic movement was tested, UI interactions lacked formal correctness guarantees, risking unhandled menu inputs or missed actions.
**Action:** Always ensure that edge-triggered UI actions and secondary modifier mappings are covered alongside primary game controls to guarantee deterministic input generation. Use table-driven testing to exhaustively test modifier variations.
