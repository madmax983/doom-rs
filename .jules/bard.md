...
## 2024-05-18 - [Added Executable Examples to Audio Core Types]
**Confusion:** The `doom-audio` core synthesis and mixer components (`OplOperator`, `OplChannel`, `OplChip`, `PcmSample`, `MixChannel`, `Mixer`) lacked executable examples, leaving developers unsure how to manually construct or interact with them outside of `SfxMixer`.
**Clarification:** Added executable `## Examples` doc-tests to all missing public structs in `doom-audio::opl` and `doom-audio::mixer`.
