...
## 2024-06-25 - [Clarified AI Director pacing thresholds]
**Confusion:** The `doom-game::director` module was a "Black Box" with zero documentation explaining how the `AiDirector` makes decisions. Users had no idea that pacing was dynamically controlled by health thresholds.
**Clarification:** Added module-level documentation (`//!`) explaining the dynamic difficulty philosophy. Added `///` doc-tests to `AiDirector::tick` to explicitly demonstrate the exact health thresholds (> 80 for ambush, < 30 for relief).
