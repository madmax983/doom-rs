...
## 2026-03-31 - [Clarified SpriteClipHistory and module-level doom-app docs]
**Confusion:** The `sprite_clip.rs` module in `doom-renderer` contained mechanical 'getter' docs (e.g. 'Creates a new empty history') without explaining *why* it drops clips after 8, or how the renderer uses it. Additionally, `doom-app/src/lib.rs` triggered missing docs errors because it is an artificial library file used just for hosting crate-level architecture docs for the binary crate.
**Clarification:** Rewrote `sprite_clip.rs` comments in a narrative style, explaining the performance trade-off of the 8-clip hard cap (avoiding heap allocations for deep geometry overdraw) and added executable `## Examples`. Added module-level `//!` documentation to `doom-app/src/lib.rs` to explain its purpose as a dummy doc host.
