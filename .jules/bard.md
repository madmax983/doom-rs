...
## 2024-07-18 - [Clarified explicit links to re-exported items]
**Confusion:** Rustdoc throws redundant explicit link warnings when linking to a destination which is exactly the same as the link label, such as linking `[`TIC_DURATION`](doom_tui::TIC_DURATION)` when `TIC_DURATION` has been explicitly imported.
**Clarification:** To fix redundant explicit link target warnings, drop the explicit link target from the markdown brackets `[`TIC_DURATION`]` when the item is imported into the module.
