...
## 2024-05-30 - [Documented doom-app and doom-renderer::sprite_clip]
**Confusion:** The crate-level documentation for `doom-app` was completely missing, making it unclear how the orchestration logic tied the various `doom-*` subsystems together. Additionally, `doom-renderer` exposed an internal module `sprite_clip` publicly without any documentation. The rationale behind using a fixed-size `[SpriteClipStep; 8]` array instead of a `Vec` for `SpriteClipHistory` was not documented, leading to confusion about the magic number limit.
**Clarification:** Added crate-level `//!` documentation for `doom-app` to explain its role as the 'Grand Assembly'. Hid the internal `sprite_clip` module using `#[doc(hidden)]` while documenting `SpriteClipHistory` with explicit `## Examples` and detailed reasoning behind its fixed-size allocation to prevent thrashing the global allocator.
