...
## 2024-05-19 - [Added missing WeaponType & AmmoType examples]
**Confusion:** The `weapons.rs` module in `doom-types` was missing `## Examples` doc-tests for `WeaponType` and `AmmoType` enums and `WeaponType::from_num`.
**Clarification:** Added explicit `///` block comments with `## Examples` doc-tests for these public types.
