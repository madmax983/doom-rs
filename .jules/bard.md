...
**Confusion:** Module `doom-renderer::sprite_clip` and crate `doom-app` had no module-level documentation. Also `doom-renderer::sprite_clip` methods lacked documentation and examples.
**Clarification:** Added `//!` docstrings for missing modules and crate. Added `///` docstrings with executable `## Examples` doc-tests for `doom-renderer::sprite_clip` methods.
