...
## 2024-05-12 - [Added missing module docs to core application flow]
**Confusion:** Module `doom-app`, `doom-game::director`, and `doom-renderer::sprite_clip` were missing documentation, which triggered `missing_docs` errors during strict compilation. The `sprite_clip` lacked context for why ArrayVecs were used, and the AI Director lacked an explanation of its decision-making constraints.
**Clarification:** Added module level `//!` documentation explaining the architectural role of `doom-app`. Added structural docs and executable `## Examples` to `AiDirector` and `SpriteClipHistory` to clarify how tracking clipping planes and manipulating enemy spawns works.
