...
## 2024-05-13 - [Documenting doom-app and doom-renderer]
**Confusion:** Building the `doom-app` crate with `-D missing_docs` failed because the crate lacked module-level documentation (`//!`). The `doom-renderer` crate failed because the `sprite_clip` module and its `SpriteClipHistory` structure lacked documentation.
**Clarification:** Added module-level documentation to `doom-app/src/lib.rs` and the `sprite_clip` module in `doom-renderer/src/lib.rs`. Also added missing doc comments to `SpriteClipHistory` and its methods in `doom-renderer/src/sprite_clip.rs`.
