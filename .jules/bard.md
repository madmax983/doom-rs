...
## 2024-06-13 - [Documenting sprite_clip module]
**Confusion:** The `sprite_clip` module and its `SpriteClipHistory` structure lacked any documentation, causing warnings during `cargo doc`. It was unclear what the structure did or how to use its methods.
**Clarification:** Added module-level documentation explaining the purpose of tracking portal clipping planes for sprites without allocating heap memory. Added method-level documentation with executable `///` doc-tests for `new`, `push`, `last`, and `iter`.
