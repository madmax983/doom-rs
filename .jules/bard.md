...
## 2024-07-29 - [Clarified Various Missing Module Docs]
**Confusion:** Several files across `doom-app`, `doom-game`, `doom-renderer`, `doom-map`, and `doom-audio` were entirely missing `//!` module-level documentation. This made it difficult for developers to understand the high-level intent of the modules, showing up as warnings during `cargo doc`.
**Clarification:** Added narrative-driven `//!` blocks to these files to explain *why* they exist and how they fit into the overall architecture, such as how `doom-app` acts as the orchestrator or how `AiDirector` manages pacing.
