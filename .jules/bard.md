...
## 2024-07-29 - [Added missing story for AiDirector]
**Confusion:** The `doom-game::director` module had no comments. It was completely unclear what its purpose was or how `tick` determined the difficulty adjustments based on player health.
**Clarification:** Added module-level `//!` narrative explaining the "dungeon master" concept. Added struct and enum `///` documentation, and an executable doc-test for `tick` demonstrating the health thresholds for ambush and relief.
