...
## 2024-06-05 - [Added missing doc tests for core math and graph tools]
**Confusion:** Essential types and tools in `doom-types` (`angle`, `fixed`) and `doom-map` (`SectorGraph`) had missing examples. Users didn't have clear instructions on how to use trigonometric bounds correctly or topological search API.
**Clarification:** Added executable `## Examples` doc tests to missing public methods in `angle.rs`, `fixed.rs`, and `graph.rs`, improving documentation completeness and code examples.
