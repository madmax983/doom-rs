...
## 2024-11-20 - [Clarified core system module roles]
**Confusion:** Several core modules (`doom-renderer/src/sprite_clip.rs`, `doom-game/src/sound_prop.rs`, `doom-game/src/stats.rs`, `doom-game/src/director.rs`, `doom-game/src/movers.rs`, and the `doom-app/src/lib.rs` file) lacked high-level module stories (`//!`). This made it hard to grasp the overarching architecture without tracing individual struct definitions.
**Clarification:** Added module-level documentation (`//!`) to explain the "why" and "how" of each module. For instance, `sprite_clip` was clarified as a zero-allocation occlusion tracker, `sound_prop` was explained as an acoustic event queue system completely decoupled from WAD lumps, and `doom-app/lib.rs` was described as the container for orchestrator subcomponents.
