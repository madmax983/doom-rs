...
## 2024-05-19 - [Added missing module docs]
**Confusion:** The "Black Box": Many modules in `doom-game` and `doom-renderer` lacked module-level (`//!`) documentation explaining the high-level concept, making it hard for users to understand their purpose without reading the code.
**Clarification:** Added module-level documentation to `director.rs`, `movers.rs`, `sound_prop.rs`, `stats.rs`, and `sprite_clip.rs` to explain *what* these modules do and *why* they exist.
