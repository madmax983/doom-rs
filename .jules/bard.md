...
## 2024-06-12 - Missing module-level docs in doom-app and doom-renderer
**Confusion:** Missing module-level documentation in `doom-app` (crate root) and `doom-renderer::sprite_clip` caused `cargo doc` to fail with missing_docs error.
**Clarification:** Added module-level doc in `doom-app/src/lib.rs` and documented `SpriteClipHistory` methods in `doom-renderer/src/sprite_clip.rs`.
