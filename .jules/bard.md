...
## 2024-05-18 - The Unseen Director
**Confusion:** The `AiDirector` in `doom-game` wasn't documented, and its existence and behavior weren't easily visible via standard `cargo doc`. Furthermore, missing doc-comments on several types in `geom.rs`, `tic.rs`, and `sprite_clip.rs` resulted in rustdoc lints.
**Clarification:** Documented `AiDirector` and `DirectorAction`, providing usage examples showing how it affects game flow via the `tick()` method. Also added documentation for `SpriteClipHistory`, `DivLine`, `ANG180`, `ANG270`, and `tick_all_mobjs`, making sure they provide context and executable examples where appropriate.
