...
**Use SmallVec for per-frame allocations**
**Learning:** `Vec::new()` creates a heap allocation every frame in `doom-renderer`'s `render_level` for `masked_columns`. Using `SmallVec` eliminates this overhead on the hot path for the vast majority of scenes.
**Action:** Replace `Vec<T>` with `smallvec::SmallVec<[T; N]>` for short-lived collections constructed per-frame, adjusting dependencies as needed.
