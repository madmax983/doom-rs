...
**Avoid intermediate `Vec<char>` allocations by using `char_indices`:**
**Learning:** `content.chars().collect::<Vec<char>>()` unnecessarily allocates heap memory for string parsing.
**Action:** Replace `Vec<char>` parsing with `char_indices().peekable()`, allowing zero-copy string slice bounds calculations and saving O(N) allocations.
