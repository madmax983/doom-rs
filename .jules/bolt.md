...
**Avoid Heap Allocations for Small Transient Collections**
**Learning:** Using `Vec::new()` inside hot functions like physics updates (e.g., `move_spechit`) causes significant allocator churn.
**Action:** Replace `Vec` with `smallvec::SmallVec` with a modest on-stack capacity matching historical limits (e.g. vanilla `MAXSPECHITS` is 8) to completely eliminate heap allocations on the hot path.
