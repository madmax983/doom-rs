...
**Eliminate Heap Allocations in Hot Paths with SmallVec**
**Learning:** Hot paths like movement and collision checks often allocate short-lived tracking vectors (e.g. `seen` lists).
**Action:** Replace `Vec::new()` with `SmallVec` with a reasonable inline capacity (e.g. `[usize; 32]`) for local tracking sets in hot movement routines to eliminate heap allocations per frame.
