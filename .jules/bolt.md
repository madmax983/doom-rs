...
**[Pre-allocate Masked Columns in Hot Render Path]**
**Learning:** `Vec::new()` is called every frame to store `masked_columns`, leading to unnecessary dynamic allocations during rendering.
**Action:** Replace `Vec::new()` with `Vec::with_capacity(SCREEN_W)` because a single frame render generally produces up to SCREEN_W masked columns.
