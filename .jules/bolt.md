...
**Use SmallVec in sectors_by_tag**
**Learning:** Returning `Vec<usize>` from helper functions used heavily during linedef dispatch loops causes excessive heap allocation churn. `smallvec` can be used to keep these short arrays on the stack.
**Action:** Use `smallvec::SmallVec` for commonly short sequences returned by helper methods on hot paths.
