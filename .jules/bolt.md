...
**[Apply Save Overwrite Optimization]
**Learning:** During game state loading, using `*gs = payload.state.clone();` causes a massive, unnecessary full-struct allocation for the game state, when `payload` is already owned and can just be assigned via `*gs = payload.state;`.
**Action:** Always consume owned types when assigning into references instead of cloning if the source is no longer needed.
