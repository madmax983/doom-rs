...
**Pre-allocating Vecs to remove .collect() churn**
**Learning:** The `.filter_map(...).collect()` pattern, while idiomatic, forces heap reallocations when chained into an unbounded vector collection on a performance hot path.
**Action:** On rendering hot paths (e.g., Sprite sorting), replace the iterator chain with `Vec::with_capacity()` using `iter.size_hint().0` and a `for` loop to build the vector without resizing.
