...
**[Eliminate heap allocations in move_spechit via SmallVec]
**Learning:** In hot paths like collision detection (move_spechit), repeatedly allocating short-lived vectors for tracking state (seen) and results (spechit) causes high heap churn.
**Action:** Use smallvec::SmallVec to allocate small numbers of elements on the stack, falling back to the heap only when necessary, which drastically cuts down on allocations per frame.
