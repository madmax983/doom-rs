...
**Optimization: Pre-allocate MapAnalyzer collections**
**Learning:** Instantiating `HashSet` and `HashMap` inside DFS loops without a predefined capacity forces repeated heap reallocations when parsing large topologies.
**Action:** Always leverage known graph bounds (like `adjacency_list.len()`) to instantiate tracking collections using `.with_capacity()` to eliminate dynamic allocation overhead.
