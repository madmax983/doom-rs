...
**Eliminate per-tic allocations in DemoPlayer**
**Learning:** Found that `DemoPlayer::next_tic_cmds` returned an `Option<Vec<DemoTicCmd>>` by cloning an internal vector on every simulated tic. Returning `Option<&[DemoTicCmd]>` avoids an allocation per tic completely while playing demos, which happen many times a second.
**Action:** Replace `Option<Vec<T>>` with `Option<&[T]>` for getters in hot loops, accessing internal state using `.get(i).map(Vec::as_slice)` instead of `.clone()`.
