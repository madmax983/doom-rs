...
## 2026-07-28 - move_spechit Array Allocations
**Learning:** `move_spechit` inside the movement logic allocated heap memory every tick using `Vec::new()` for tracking crossed `spechit` and `seen` blockmap lines.
**Action:** Replace `Vec::new()` with `smallvec::SmallVec` for `spechit` and `seen` within hot trace/movement routines, since lines hit or checked in a tick are generally extremely few.
