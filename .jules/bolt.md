...
**[SaveGame Ownership Transfer]
**Learning:** Overwriting the game state pointer (`*gs = payload.state.clone()`) inside `apply_save` caused a massive, expensive heap allocation during deserialization.
**Action:** Transferred ownership of the large state struct directly into the pointer by modifying the function signature to take `SaveGame` by value instead of reference (`*gs = payload.state`), eliminating the clone.
