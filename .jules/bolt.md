...
**Avoid Vec intermediate allocations when collecting sectors**
**Learning:** `sectors_by_tag` would create an intermediate `Vec<usize>` each time it is called. Changing this to return `impl Iterator<Item = usize> + '_` allows iterators to be yielded rather than needing to build intermediate data arrays.
**Action:** Remove `collect()` locally when iterating.
