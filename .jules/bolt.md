...
## 2026-08-03 - Pre-allocate masked_columns
**Learning:** Initializing `Vec::new()` for masked_columns on the hot rendering path caused unnecessary dynamic allocations during masked texture processing.
**Action:** Use `Vec::with_capacity(64)` based on the heuristic that scenes frequently render masked columns, eliminating intermediate heap allocations.
