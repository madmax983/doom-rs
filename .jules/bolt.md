...
## 2026-08-01 - Zero-cost move_spechit
**Learning:** Returning `Vec::new()` directly from `move_spechit` caused a dynamic heap allocation for every entity movement check, even though the returned array was extremely small and mostly empty. Using `SmallVec` with a small bounds limit eliminates heap allocation entirely.
**Action:** Replace `Vec<usize>` with `smallvec::SmallVec<[usize; N]>` where the capacity is known to be tiny on hot paths, especially collision detection inside the tick loop.
