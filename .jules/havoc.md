...
**Havoc: Bounds-checking allocations in texture lump parsing**
**Learning:** Uncapped allocations driven by input in the PNAMES and TEXTURE lumps can cause AddressSanitizer/allocator Out-Of-Memory errors.
**Action:** Use `.min(data.len() / entry_size)` when reserving memory based on input-controlled sizes like `num_names` or `patch_count` in texture parsers.
