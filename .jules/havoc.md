...
**Havoc: Recursive DFS stack overflow**
**Learning:** `analyzer.chokepoints()` recursive DFS causes a stack overflow on highly nested topologies, effectively crashing the program on malicious or highly segmented input maps. The call stack scales linearly with map depth.
**Action:** Use an iterative approach with an explicit `Vec` stack instead of function recursion to avoid blowing the call stack.
