...
## 🔍 Gap Analysis
- **Current State:** The game hardcodes default behaviors (e.g., audio volumes are fixed or not adjustable via UI). The settings menu is visually present but functionally a stub.
- **Standard Libs / Market:** Most games or applications manage configurations locally in a standard user directory. Implementing this is a standard and expected pattern for any modern source port or engine clone.

## ✅ Acceptance Criteria
- Must introduce a Configuration state module that saves and loads from disk.
- Must implement functional actions for the existing Options items (`Sound Volume`, `Music Volume`, `Controls`).
- Must wire the UI changes to the corresponding backend systems.
- Must gracefully handle file I/O errors and corrupted config files without crashing.
