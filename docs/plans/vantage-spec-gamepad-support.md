...
## 🔍 Gap Analysis
- **Current State:** Input is hardcoded to keyboard events. The internal input abstraction already models the actual hardware inputs correctly, making the backend ready.
- **Standard Libs / Market:** Cross-platform gamepad support handling standard console controller mappings natively is the industry standard.

## ✅ Acceptance Criteria
- Must implement a new input polling thread or hook into the main loop using a standard input handling mechanism.
- Must translate gamepad stick axes to the normalized range expected by the game's movement fields.
