...
## 🔍 Gap Analysis
- **Current State:** The Options menu has a "Controls" placeholder item (a no-op). Key mappings are likely statically defined in the input handling module. Gamepad support  is being implemented but lacks dynamic rebinding.
- **Standard Libs / Market:** Almost all PC games provide key rebinding. This involves maintaining a dynamic mapping between semantic actions and input events instead of matching hardcoded keys.

## ✅ Acceptance Criteria
- Must implement a new 'Customize Controls' menu page accessible from the Options menu.
...
- Must serialize the updated keymap to the standard configuration format (or equivalent persistent config).
