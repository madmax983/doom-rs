...
impl SpriteClipHistory {
    ...
    ///
    /// If the history is already at capacity (8 items), the new step is silently
    /// dropped to prevent heap allocations. In Doom, it's extremely rare for a single
    /// column of a sprite to be clipped by more than 8 distinct portal boundaries.
    ///
    /// ## Examples
    ///
    /// ```
    ...
    /// assert!(history.last().is_none());
    ///
    /// history.push(SpriteClipStep { depth: 50.0, row: 10, silhouette_height: 64.0 });
    ...
    /// Returns an iterator over all valid clipping steps in chronological order.
    ///
    /// ## Examples
    ///
    /// ```
    ...
    /// history.push(SpriteClipStep { depth: 20.0, row: 15, silhouette_height: 0.0 });
    ///
