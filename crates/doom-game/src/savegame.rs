...
mod tests {
    ...
    // --- Havoc Test: Description is invalid UTF-8 doesn't panic and loads lossily ---
    #[test]
    fn havoc_test_invalid_utf8_description() {
        ...
        // The save file should load successfully despite invalid UTF-8
        let loaded = load_game(&data).expect("load must succeed with invalid utf8 description");
        let desc = &loaded.header.description;
        let desc_str = String::from_utf8_lossy(desc);
        assert!(desc_str.contains("y Cool"));
    }
