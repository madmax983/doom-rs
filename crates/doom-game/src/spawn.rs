    ...
    for thing in &level.things {
        ...
        if let Some(spawned) = gs.mobjslab.get(handle) {
            if spawned.flags & flags::MF_COUNTKILL != 0 {
                gs.level_stats.total_kills += 1;
            }
            if spawned.flags & flags::MF_COUNTITEM != 0 {
                gs.level_stats.total_items += 1;
            }
        }
    }
...
mod tests {
    ...
    fn spawn_counts_kills_and_items() {
        ...
        let mut gs = GameState::new("E1M1");
        spawn_level_things(&mut gs, &level, Skill::Medium, false);
        assert_eq!(gs.level_stats.total_kills, 1, "Trooper should count as a kill");
        assert_eq!(gs.level_stats.total_items, 1, "HealthBonus should count as an item");
    }
    ...
    fn spawn_counts_multiple_kills() {
        ...
        let mut gs = GameState::new("E1M1");
        spawn_level_things(&mut gs, &level, Skill::Medium, false);
        assert_eq!(gs.level_stats.total_kills, 3, "All three monsters should be counted");
    }
    ...
    fn spawn_empty_level() {
        ...
        assert_eq!(gs.level_stats.total_kills, 0);
        assert_eq!(gs.level_stats.total_items, 0);
    }
