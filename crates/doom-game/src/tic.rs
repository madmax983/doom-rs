...
/// Executes a single logical simulation tic for all mobjs (map objects/actors) currently in the game.
///
/// This handles movement, state transitions, collision detection, and logic updates for every
/// monster, projectile, and pickup.
///
/// ## Implementation Details
///
/// It processes actors in a deterministic order by their unique `generation` ID,
/// ensuring that missiles spawned during this tic are also handled immediately.
...
fn p_death_think(gs: &mut GameState, cmd: TicCmd, level: Option<&mut Level>) {
    ...
    let on_ground = match level.as_deref() {
        Some(lv) => {
            match crate::movement::support_state_at(&gs.mobjslab, handle, final_x, final_y, lv) {
                Some((floorz, _)) => {
                    if mz > floorz {
                        false
                    } else {
...
fn p_z_movement_mobj(gs: &mut GameState, handle: MobjHandle, level: Option<&Level>) {
    ...
    let Some((floorz, _)) = crate::movement::support_state_at(&gs.mobjslab, handle, x, y, lv)
    else {
        return;
    };
...
fn p_xy_movement_mobj(gs: &mut GameState, handle: MobjHandle, level: Option<&Level>) {
    ...
    if let Some(lv) = level {
        ...
        if let Some((floorz, _)) = crate::movement::support_state_at(&gs.mobjslab, handle, x, y, lv)
        {
            ...
            let center_floor = lv.floor_at_fixed(x.raw(), y.raw()).unwrap_or(floorz);
