...
/// Extracted logic from `p_xy_movement_mobj` to reset a skull-fly charge.
fn reset_skullfly_mobj(gs: &mut GameState, handle: MobjHandle, level: Option<&Level>) {
    let spawn_state = {
...
fn p_xy_movement_mobj(gs: &mut GameState, handle: MobjHandle, level: Option<&Level>) {
    ...
    if momx0 == Fixed16_16::ZERO && momy0 == Fixed16_16::ZERO {
        if flags0 & flags::MF_SKULLFLY != 0 {
            reset_skullfly_mobj(gs, handle, level);
        }
        return;
    }
