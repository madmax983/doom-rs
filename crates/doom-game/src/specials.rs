...
fn get_back_sector(level: &Level, left_sidedef: i16) -> Option<usize> {
    level
        .sidedefs
...
fn player_has_blue_key(gs: &GameState) -> bool {
    gs.player.has_key(crate::player::KEY_BLUE_CARD)
        || gs.player.has_key(crate::player::KEY_BLUE_SKULL)
}

fn player_has_red_key(gs: &GameState) -> bool {
    gs.player.has_key(crate::player::KEY_RED_CARD)
        || gs.player.has_key(crate::player::KEY_RED_SKULL)
}

fn player_has_yellow_key(gs: &GameState) -> bool {
    gs.player.has_key(crate::player::KEY_YELLOW_CARD)
