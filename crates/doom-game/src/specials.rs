...
fn check_door_lock(gs: &GameState, special: u16) -> bool {
    match special {
        26 | 99 | 133 => {
...
fn activate_doors(
    ...
    let get_back_sector = || -> Option<usize> {
        level
            .sidedefs
    ...
    match special {
        ...
        1 => {
            let Some(sector_idx) = get_back_sector() else {
                return;
            };
        ...
        // Types 99, 134, 136: Key door open-stay (normal speed).
        99 | 134 | 136 => {
            let Some(sector_idx) = get_back_sector() else {
                return;
            };
        ...
        // Types 133, 135, 137: Key door open-stay (blazing).
        133 | 135 | 137 => {
            let Some(sector_idx) = get_back_sector() else {
                return;
            };
