...
pub fn p_player_in_special_sector(gs: &mut GameState, level: &mut Level) {
    ...
    match crate::state::SectorDamageType::from_repr(special) {
        Some(crate::state::SectorDamageType::Hellslime) => {
            // HELLSLIME DAMAGE
            if !has_ironfeet && on_period {
                hurt(gs, 10);
            }
        }
        Some(crate::state::SectorDamageType::Nukage) => {
            // NUKAGE DAMAGE
            if !has_ironfeet && on_period {
                hurt(gs, 5);
            }
        ...
        Some(crate::state::SectorDamageType::GodExit) => {
