...
pub struct MobjInfo {
    ...
    /// Over-kill (gib / xdeath) animation start state. Vanilla `info.c`
    /// `xdeathstate`. `P_KillMobj` uses this instead of `death_state` when the
    /// victim's `health < -spawn_health` and this is not `S_NULL`. Most actors
    /// have `S_NULL` (no gib chain); of the E1 demo monsters, MT_POSSESSED,
    /// MT_SHOTGUY and MT_TROOP have gib chains (the pinky demon does not).
...
pub static MOBJINFO: [MobjInfo; 76] = [
    ...
    MobjInfo {
        ...
        pain_state: sn(ids::S_NULL),
        death_state: sn(ids::S_NULL),
        // Vanilla MT_PLAYER has S_PLAY_XDIE1, but doom-rs stubs player death
        // (death_state = S_NULL) and has no S_PLAY_XDIE states; keep S_NULL.
        xdeath_state: sn(ids::S_NULL),
        melee_state: sn(ids::S_NULL),
        missile_state: sn(ids::S_NULL),
        raise_state: sn(ids::S_NULL),
    ...
    MobjInfo {
        ...
        xdeath_state: sn(ids::S_POSS_XDIE1),
        melee_state: sn(ids::S_NULL),
    ...
    MobjInfo {
        ...
        xdeath_state: sn(ids::S_SPOS_XDIE1),
        melee_state: sn(ids::S_NULL),
    ...
    MobjInfo {
        ...
        xdeath_state: sn(ids::S_TROO_XDIE1),
    ...
    MobjInfo {
        ...
        pain_state: sn(ids::S_NULL),
        death_state: sn(ids::S_NULL),
        // Vanilla MT_WOLFSS has S_SSWV_XDIE1, but doom-rs stubs WolfSS with
        // Trooper states (death_state = S_NULL) and has no SSWV states; S_NULL.
        xdeath_state: sn(ids::S_NULL),
        melee_state: sn(ids::S_NULL),
        missile_state: sn(ids::S_NULL),
        raise_state: sn(ids::S_NULL),
