...
fn make_tagged_sector_level(floor: i16, ceil: i16, tag: u16, ld_special: u16) -> doom_map::Level {
...
fn floor_mover_with_crush_flag_set() {
    ...
    let _crush_dmg: i32 = if gs.movers.active_floors[0].crush == crate::state::CrushBehavior::Crush
    {
        10
    } else {
        0
...
fn ev_floor_raise_to_lowest_ceiling_works() {
    ...
    ev_floor_raise_to_lowest_ceiling(&mut gs, &level, 1, 1, crate::state::CrushBehavior::NoCrush);
    assert_eq!(gs.movers.active_floors.len(), 1);
    assert_eq!(
        gs.movers.active_floors[0].target_height, 96,
        "lowest adj ceil = 96"
