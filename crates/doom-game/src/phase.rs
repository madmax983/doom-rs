...
impl MapId {
    ...
    fn next_map_doom1(&self, exit_type: ExitType) -> Option<MapId> {
        let ep = self.episode;
        let map = self.map;
        let is_secret = matches!(exit_type, ExitType::Secret);
...
impl GamePhaseController {
    ...
    fn tick_playing(&mut self, game_state: &mut GameState) {
        if let Some(exit_req) = game_state.exit_request.take() {
            let exit_type = match exit_req {
                ExitRequest::Secret => ExitType::Secret,
                _ => ExitType::Normal,
            };
            let is_secret = matches!(exit_type, ExitType::Secret);
            let stats = game_state.compute_intermission_stats();

            if self.current_map.is_final_map() && !is_secret {
                // Final map -> Finale
