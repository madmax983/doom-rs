...
/// The type of level exit taken by the player.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ExitType {
    /// A normal exit leading to the next sequential map.
    Normal,
    /// A secret exit leading to a secret map.
    Secret,
}
...
impl MapId {
    ...
    ///
    /// `exit_type`: Indicates if the exit was normal or secret.
    ///
    ...
    /// - MAP32 normal -> MAP16
    /// - MAP30 -> None (game end)
    pub fn next_map(&self, exit_type: ExitType) -> Option<MapId> {
        if self.is_doom2() {
    ...
    fn next_map_doom1(&self, exit_type: ExitType) -> Option<MapId> {
        let ep = self.episode;
        let map = self.map;

        // Secret exits: ExM3 -> ExM9
        if exit_type == ExitType::Secret && map == 3 {
    ...
    fn next_map_doom2(&self, exit_type: ExitType) -> Option<MapId> {
        let map = self.map;
        ...
        // MAP15 + secret -> MAP31
        if map == 15 && exit_type == ExitType::Secret {
            return Some(MapId::doom2(31));
        }

        // MAP31 + secret -> MAP32
        if map == 31 && exit_type == ExitType::Secret {
...
impl GamePhaseController {
    ...
    fn tick_playing(&mut self, game_state: &mut GameState) {
        if let Some(exit_req) = game_state.exit_request.take() {
            ...
            } else if let Some(next) = self.current_map.next_map(exit_type) {
...
mod tests {
    ...
    fn next_map_e1m1_normal() {
        ...
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(1, 2)));
    }

    #[test]
    fn next_map_e1m2_normal() {
        let id = MapId::new(1, 2);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(1, 3)));
    }

    #[test]
    fn next_map_e1m3_normal() {
        let id = MapId::new(1, 3);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(1, 4)));
    }

    #[test]
    fn next_map_e1m3_secret() {
        let id = MapId::new(1, 3);
        assert_eq!(id.next_map(ExitType::Secret), Some(MapId::new(1, 9)));
    }

    #[test]
    fn next_map_e1m4_normal() {
        let id = MapId::new(1, 4);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(1, 5)));
    }

    #[test]
    fn next_map_e1m5_normal() {
        let id = MapId::new(1, 5);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(1, 6)));
    }

    #[test]
    fn next_map_e1m6_normal() {
        let id = MapId::new(1, 6);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(1, 7)));
    }

    #[test]
    fn next_map_e1m7_normal() {
        let id = MapId::new(1, 7);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(1, 8)));
    }

    #[test]
    fn next_map_e1m8_ends_episode() {
        let id = MapId::new(1, 8);
        assert_eq!(id.next_map(ExitType::Normal), None);
    }
    ...
    fn next_map_e2m1_normal() {
        ...
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(2, 2)));
    }

    #[test]
    fn next_map_e2m3_secret() {
        let id = MapId::new(2, 3);
        assert_eq!(id.next_map(ExitType::Secret), Some(MapId::new(2, 9)));
    }
    ...
    fn next_map_e2m9_returns_to_e2m4() {
        ...
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(2, 4)));
    }
    ...
    fn next_map_e3m1_normal() {
        ...
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(3, 2)));
    }

    #[test]
    fn next_map_e3m3_secret() {
        let id = MapId::new(3, 3);
        assert_eq!(id.next_map(ExitType::Secret), Some(MapId::new(3, 9)));
    }
    ...
    fn next_map_e3m9_returns_to_e3m4() {
        ...
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::new(3, 4)));
    }
    ...
    fn next_map_map01_normal() {
        ...
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::doom2(2)));
    }

    #[test]
    fn next_map_map14_normal() {
        let id = MapId::doom2(14);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::doom2(15)));
    }

    #[test]
    fn next_map_map15_normal() {
        let id = MapId::doom2(15);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::doom2(16)));
    }

    #[test]
    fn next_map_map15_secret() {
        let id = MapId::doom2(15);
        assert_eq!(id.next_map(ExitType::Secret), Some(MapId::doom2(31)));
    }

    #[test]
    fn next_map_map16_normal() {
        let id = MapId::doom2(16);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::doom2(17)));
    }

    #[test]
    fn next_map_map29_normal() {
        let id = MapId::doom2(29);
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::doom2(30)));
    }
    ...
    fn next_map_map31_secret_goes_to_map32() {
        let id = MapId::doom2(31);
        assert_eq!(id.next_map(ExitType::Secret), Some(MapId::doom2(32)));
    }
    ...
    fn secret_exit_on_non_secret_map_is_normal() {
        ...
        let id = MapId::new(1, 1);
        assert_eq!(id.next_map(ExitType::Secret), Some(MapId::new(1, 2)));
    }
    ...
    fn e1m8_secret_exit_still_goes_to_secret() {
        ...
        // Instead, next_map(ExitType::Secret) for E1M8 returns None (map==8 -> None).
    ...
    fn doom2_linear_progression_map20_to_map21() {
        ...
        assert_eq!(id.next_map(ExitType::Normal), Some(MapId::doom2(21)));
    }
    ...
    fn doom2_map32_secret_exit_still_goes_to_map16() {
        ...
        let id = MapId::doom2(32);
        assert_eq!(id.next_map(ExitType::Secret), Some(MapId::doom2(16)));
    }
