...
impl MapId {
    ...
    /// - MAP32 normal -> MAP16
    /// - MAP30 -> None (game end)
    pub fn next_map(&self, exit_type: ExitRequest) -> Option<MapId> {
        if self.is_doom2() {
    ...
    fn next_map_doom1(&self, exit_type: ExitRequest) -> Option<MapId> {
        let ep = self.episode;
        let map = self.map;

        // Secret exits: ExM3 -> ExM9
        if exit_type == ExitRequest::Secret && map == 3 {
    ...
    fn next_map_doom2(&self, exit_type: ExitRequest) -> Option<MapId> {
        let map = self.map;
        ...
        // MAP15 + secret -> MAP31
        if map == 15 && exit_type == ExitRequest::Secret {
            return Some(MapId::doom2(31));
        }

        // MAP31 + secret -> MAP32
        if map == 31 && exit_type == ExitRequest::Secret {
