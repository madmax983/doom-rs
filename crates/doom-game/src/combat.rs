...
pub fn p_radius_attack(
    ...
    // Pre-allocate buffers for ray tracing to avoid allocations inside the hot loop.
    let num_lines = level.map(|l| l.linedefs.len()).unwrap_or(0);
    let mut tested_lines = Vec::with_capacity(num_lines);
