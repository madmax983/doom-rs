...
pub fn damage_mobj_source(
    ...
    if target == gs.player.handle
        && source != gs.player.handle
        && source != MobjHandle::NULL
        && gs.nemesis.try_promote(source, damage)
    {
        if let Some(nemesis_mo) = gs.mobjslab.get_mut(source) {
            nemesis_mo.health = nemesis_mo.health.saturating_add(gs.nemesis.health_boost);
        }
    }
    ...
    if new_health <= 0 {
        ...
        #[cfg(feature = "nemesis")]
        if let Some(reward) = gs.nemesis.defeat(target) {
            let new_health = gs.player.health() + reward;
            gs.set_player_health_capped(new_health, doom_types::limits::MAX_HEALTH);
        }
