...
mod tests {
    ...
    #[test]
    fn p_spawn_blood_spawns_blood_mobj_with_correct_states() {
        let mut gs = make_game_state();
        let x = 100 << 16;
        let y = 100 << 16;
        let z = 50 << 16;

        // Damage 10, should be S_BLOOD2
        p_spawn_blood(&mut gs, None, x, y, z, 10);
        let mut found_blood2 = false;
        for h in gs.mobjslab.iter_handles() {
            let mo = gs.mobjslab.get(h).expect("value must exist in test");
            if mo.kind == MobjKind::Blood && mo.state.0 == ids::S_BLOOD2 {
                found_blood2 = true;
            }
        }
        assert!(found_blood2, "Expected S_BLOOD2 for damage 10");

        // Damage 8, should be S_BLOOD3
        p_spawn_blood(&mut gs, None, x, y, z, 8);
        let mut found_blood3 = false;
        for h in gs.mobjslab.iter_handles() {
            let mo = gs.mobjslab.get(h).expect("value must exist in test");
            if mo.kind == MobjKind::Blood && mo.state.0 == ids::S_BLOOD3 {
                found_blood3 = true;
            }
        }
        assert!(found_blood3, "Expected S_BLOOD3 for damage 8");

        // Damage 15, should be S_BLOOD1 (unchanged by the specific logic)
        p_spawn_blood(&mut gs, None, x, y, z, 15);
        let mut found_blood1 = false;
        for h in gs.mobjslab.iter_handles() {
            let mo = gs.mobjslab.get(h).expect("value must exist in test");
            if mo.kind == MobjKind::Blood && mo.state.0 == ids::S_BLOOD1 {
                found_blood1 = true;
            }
        }
        assert!(found_blood1, "Expected S_BLOOD1 (default) for damage 15");
    }
}
