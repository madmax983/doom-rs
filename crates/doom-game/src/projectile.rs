...
mod tests {
    ...
    fn rocket_has_correct_stats() {
        let info = projectile_info(MobjKind::Rocket).expect("Expected successful result in test");
    ...
    fn all_projectiles_have_missile_flag() {
        ...
        for kind in projectile_kinds {
            let info = projectile_info(kind).expect("Expected successful result in test");
            assert_ne!(
    ...
    fn spawn_missile_creates_projectile_aimed_at_target() {
        ...
        let proj = gs
            .mobjslab
            .get(proj_h.expect("Expected successful result in test"))
