...
pub enum PlayerFiringStatus {
    Firing,
    NotFiring,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PlayerInvulnerability {
    Invulnerable,
    Vulnerable,
}
...
impl FaceState {
    ...
    pub fn tick(
        &mut self,
        health: i32,
        firing_status: PlayerFiringStatus,
        ...
        if firing_status == PlayerFiringStatus::Firing {
            self.firing_tics = self.firing_tics.saturating_add(1);
        } else {
            self.firing_tics = 0;
        }
        self.was_firing = firing_status == PlayerFiringStatus::Firing;
        ...
        // 2. God mode
        if invulnerability == PlayerInvulnerability::Invulnerable {
...
mod tests {
    ...
    fn tick_simple(face: &mut FaceState, health: i32) {
        face.tick(
            health,
            PlayerFiringStatus::NotFiring,
            PlayerInvulnerability::Vulnerable,
            None,
        );
    }
    ...
    fn death_takes_highest_priority() {
        ...
        face.tick(
            0,
            PlayerFiringStatus::NotFiring,
            PlayerInvulnerability::Invulnerable,
            None,
        );
