...
/// Describes whether the player is currently firing their weapon.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// Describes whether the player currently has the invulnerability power-up active.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Invulnerability {
    /// The invulnerability power-up is active.
    Active,
    /// The invulnerability power-up is not active.
    Inactive,
}
...
impl FaceState {
    ...
    pub fn tick(
        ...
        if is_firing == WeaponFiring::Firing {
...
mod tests {
    ...
    fn tick_simple(face: &mut FaceState, health: i32) {
        face.tick(health, WeaponFiring::Idle, Invulnerability::Inactive, None);
    }
    ...
    fn death_takes_highest_priority() {
        ...
        face.tick(0, WeaponFiring::Idle, Invulnerability::Active, None);
    ...
    fn god_mode_overrides_normal() {
        let mut face = make_face();
        face.tick(100, WeaponFiring::Idle, Invulnerability::Active, None);
