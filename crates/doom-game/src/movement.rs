...
#[must_use]
/// ⚡ Bolt Optimization:
/// Replacing dynamic `Vec` allocations with stack-allocated `SmallVec` for tracking
/// crossed special lines (`spechit`) prevents unnecessary heap allocations on the hot path.
/// Since monsters rarely cross more than 1 or 2 special lines during a single movement step,
/// an 8-element capacity safely keeps these collections completely on the stack.
pub fn move_spechit(
    slab: &MobjSlab,
    handle: MobjHandle,
    new_x: Fixed16_16,
    ...
    // ⚡ Bolt Optimization: Keeps the collision cell filter `seen` list on the stack.
