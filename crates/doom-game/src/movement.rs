...
///
/// ⚡ Bolt Optimization: Uses `SmallVec` instead of `Vec` for `spechit` and `seen`
/// arrays to eliminate dynamic heap allocations. `move_spechit` is called heavily on
/// the hot path during collision detection, and keeping these typically small arrays
/// on the stack measurably improves performance and reduces memory fragmentation.
#[must_use]
pub fn move_spechit(
    slab: &MobjSlab,
    handle: MobjHandle,
