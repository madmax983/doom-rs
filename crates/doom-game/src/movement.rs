...
///
/// Returns `SmallVec` to eliminate dynamic heap allocations on this extremely hot path.
/// Most spechit lists are very small (< 4), so an inline capacity of 8 avoids the heap almost entirely.
#[must_use]
pub fn move_spechit(
    slab: &MobjSlab,
    handle: MobjHandle,
