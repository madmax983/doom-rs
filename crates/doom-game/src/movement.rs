...
fn collect_slide_intercepts(
    ...
    dx: i32,
    dy: i32,
    out: &mut smallvec::SmallVec<[(i32, usize); 8]>,
) {
...
fn slide_traverse(
    ...
    dx: i32,
    dy: i32,
    scratch: &mut smallvec::SmallVec<[(i32, usize); 8]>,
    ...
    for &(frac, ld_idx) in scratch.iter() {
        ...
        if !ld.is_two_sided() {
            ...
            if openrange < mo_height
                || opentop - mo_z < mo_height
                || openbottom - mo_z > 24 * FRACUNIT
            {
                blocking = true;
            } else {
...
pub fn p_slide_move_vanilla(
    ...
    let mut scratch: smallvec::SmallVec<[(i32, usize); 8]> = smallvec::SmallVec::new();
    let mut hitcount = 0;
