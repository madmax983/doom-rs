...
mod tests {
    ...
    fn mixer_play_on_channel_restarts_in_place() {
        let mut mixer = SfxMixer::new();
        let ch = mixer
            .play(1, std::sync::Arc::<[u8]>::from(vec![200u8; 1000]), 1.0, 0.0, SfxPriority::Medium)
            .expect("should allocate a channel");

        let replaced =
            mixer.play_on_channel(ch, 2, vec![220u8; 1000].into(), 0.4, -0.5, SfxPriority::High);
    ...
    fn weapon_sound_survives_monster_attack_flood() {
        let mut mixer = SfxMixer::new();
        for i in 0..MAX_CHANNELS {
            mixer.play(i as u16, vec![128u8; 1000].into(), 1.0, 0.0, SfxPriority::Medium);
        }
        assert_eq!(mixer.active_count(), MAX_CHANNELS);
    ...
    fn weapon_sound_survives_monster_wake_flood() {
        let mut mixer = SfxMixer::new();
        for i in 0..MAX_CHANNELS {
            mixer.play(i as u16, vec![128u8; 1000].into(), 1.0, 0.0, SfxPriority::High);
        }
        assert_eq!(mixer.active_count(), MAX_CHANNELS);
    ...
    fn monster_attack_cannot_steal_weapon_channel() {
        let mut mixer = SfxMixer::new();
        for i in 0..MAX_CHANNELS {
            mixer.play(i as u16, vec![128u8; 1000].into(), 1.0, 0.0, SfxPriority::Weapon);
        }
        assert_eq!(mixer.active_count(), MAX_CHANNELS);
