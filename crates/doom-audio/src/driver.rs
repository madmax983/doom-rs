    ...
    fn havoc_loom_deadlock_test() {
        loom::model(|| {
            ...
            let t1 = thread::spawn(move || {
                let mut m = mixer1.lock().expect("Expected successful result in test");
                m.stop_all();
            });

            let t2 = thread::spawn(move || {
                let mut m = mixer2.lock().expect("Expected successful result in test");
                let mut buf = [0.0; 2];
                m.mix(&mut buf, 44100);
            });

            t1.join().expect("Expected successful result in test");
            t2.join().expect("Expected successful result in test");
        });
    }
}
