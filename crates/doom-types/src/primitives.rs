...
///
/// let dark = Brightness::new(0).expect("valid primitive creation must succeed");
...
impl Brightness {
    ...
    /// ```
    /// use doom_types::primitives::Brightness;
    /// let b = Brightness::new(128).expect("valid primitive creation must succeed");
    /// assert_eq!(b.raw(), 128);
    /// ```
    #[inline]
...
/// let secret = SectorSpecial::new(9).expect("valid primitive creation must succeed");
...
/// use doom_types::primitives::SectorSpecial;
///
/// let special = SectorSpecial::new(9).expect("valid primitive creation must succeed");
...
impl SkillLevel {
    ...
    ///
    /// // Valid skill level.
    /// let skill = SkillLevel::new(2).expect("valid primitive creation must succeed");
...
///
/// let p1 = PlayerNum::new(0).expect("valid primitive creation must succeed");
/// assert_eq!(p1.raw(), 0);
///
...
/// use doom_types::primitives::PlayerNum;
///
/// let player = PlayerNum::new(0).expect("valid primitive creation must succeed");
...
impl PlayerNum {
    ...
    /// ```
    /// use doom_types::primitives::PlayerNum;
    /// let p = PlayerNum::new(2).expect("valid primitive creation must succeed");
    /// assert_eq!(p.raw(), 2);
    /// ```
    #[inline]
    pub const fn raw(self) -> u8 {
...
mod tests {
    ...
    fn raw_methods_return_internal_value() {
        ...
        let s = SectorSpecial::NORMAL;
        assert_eq!(s.raw(), 0);
        let s2 = SectorSpecial::new(5).expect("valid primitive creation must succeed");
        assert_eq!(s2.raw(), 5);

        let skill = SkillLevel::UV;
        assert_eq!(skill.raw(), 3);
        let skill2 = SkillLevel::new(2).expect("valid primitive creation must succeed");
    ...
    fn skill_level_rejects_over_4() {
        assert!(SkillLevel::new(5).is_none());
        assert_eq!(SkillLevel::new(4).expect("valid primitive creation must succeed").raw(), 4);
    }
