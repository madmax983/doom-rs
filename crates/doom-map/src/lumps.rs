...
impl Sidedef {
    ...
    fn from_bytes(b: &[u8]) -> Self {
        Self {
            ...
            upper_texture: b[4..12].try_into().expect("must succeed"),
            lower_texture: b[12..20].try_into().expect("must succeed"),
            middle_texture: b[20..28].try_into().expect("must succeed"),
...
impl Sector {
    ...
    fn from_bytes(b: &[u8]) -> Self {
        Self {
            ...
            floor_flat: b[4..12].try_into().expect("must succeed"),
            ceil_flat: b[12..20].try_into().expect("must succeed"),
...
/// let reject = Reject::parse_lump(&data, 2).expect("must succeed");
...
///
/// let blockmap = Blockmap::parse_lump(&data).expect("must succeed");
