...
mod tests {
    ...
    #[test]
    fn test_from_bytes_no_panic() {
        // Before the refactor, these could panic if the byte slices were invalid lengths,
        // but `parse_fixed_records` guarantees length matches the structure.
        // We verify that Sidedef and Sector don't panic on all-zeros (which was where `try_into().unwrap()` lived).
        let sidedef_bytes = [0u8; Sidedef::BYTE_SIZE];
        let _sidedef = Sidedef::from_bytes(&sidedef_bytes);

        let sector_bytes = [0u8; Sector::BYTE_SIZE];
        let _sector = Sector::from_bytes(&sector_bytes);
    }
}
