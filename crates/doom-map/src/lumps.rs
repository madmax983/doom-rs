...
impl Sidedef {
    ...
    fn from_bytes(b: &[u8]) -> Self {
        Self {
            ...
            upper_texture: b[4..12].try_into().expect("slice must be 8 bytes long"),
            lower_texture: b[12..20].try_into().expect("slice must be 8 bytes long"),
            middle_texture: b[20..28].try_into().expect("slice must be 8 bytes long"),
...
impl Sector {
    ...
    fn from_bytes(b: &[u8]) -> Self {
        Self {
            ...
            floor_flat: b[4..12].try_into().expect("slice must be 8 bytes long"),
            ceil_flat: b[12..20].try_into().expect("slice must be 8 bytes long"),
