...
///
/// let err = UdmfMap::parse(b"namespace").expect_err("should parse fail");
...
mod tests {
    ...
    fn parse_rejects_non_string_namespace() {
        let err = UdmfMap::parse(b"namespace = 123;").expect_err("should fail parsing");
        assert!(
    ...
    fn parse_rejects_missing_namespace() {
        let err = UdmfMap::parse(b"vertex { x = 0; }").expect_err("should fail parsing");
        assert!(matches!(err, UdmfError::MissingNamespace));
    }

    #[test]
    fn parse_rejects_bare_identifier_at_root() {
        let err = UdmfMap::parse(b"namespace").expect_err("should fail parsing");
        assert!(
    ...
    fn parse_rejects_unterminated_string_literal() {
        let err = UdmfMap::parse(b"namespace = \"doom").expect_err("should fail parsing");
        assert!(
