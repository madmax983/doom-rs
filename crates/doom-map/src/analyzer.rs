...
impl DfsState {
    fn visit_node(&mut self, u: usize) {
    ...
    fn update_low_time(&mut self, u: usize, v: usize) {
        let (low_u, disc_v) = (
    ...
    fn finish_node(&mut self, u: usize, children_map: &HashMap<usize, usize>) {
                    ...
                    while let Some(&v) = neighbors_iter.next() {
                        ...
                        if !state.visited.contains(&v) {
                            *children_map.entry(u).or_default() += 1;
                            ...
                            state.update_low_time(u, v);
                        }
                    }

                    if !pushed_child {
                        state.finish_node(u, &children_map);
                    }
                }
            }
        }
