    ...
    pub fn chokepoints(&self) -> Vec<usize> {
        ...
        for &node in self.graph.adjacency_list.keys() {
            if !visited.contains(&node) {
                ...
                let Some(edges) = self.graph.adjacency_list.get(&node) else { continue; };
                let mut stack = vec![(node, edges.iter())];
                    ...
                    while let Some(&v) = neighbors_iter.next() {
                        ...
                        if !visited.contains(&v) {
                            *children_map.entry(u).or_default() += 1;
                            ...
                            if let Some(edges) = self.graph.adjacency_list.get(&v) { stack.push((v, edges.iter())); }
                            pushed_child = true;
                            break;
...
mod tests {
    ...
    fn test_chokepoints_large_linear() {
        ...
        for i in 1..10000 {
            if let Some(set) = adj.get_mut(&i) { set.insert(i - 1); }
        }
