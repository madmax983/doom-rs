...
impl<'a> MapAnalyzer<'a> {
    ...
    pub fn chokepoints(&self) -> Vec<usize> {
        let mut state = ChokepointsState::default();

        for &node in self.graph.adjacency_list.keys() {
            if !state.visited.contains(&node) {
                self.find_chokepoints_from_node(node, &mut state);
            }
        }
    ...
    fn find_chokepoints_from_node(&self, start_node: usize, state: &mut ChokepointsState) {
