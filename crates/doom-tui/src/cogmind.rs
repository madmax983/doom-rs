...
mod tests {
    ...
    fn frame_new_is_blank() {
        ...
        for y in 0..5 {
            for x in 0..10 {
                let cell = frame.get(x, y).expect("Failed to get frame cell");
    ...
    fn frame_set_get_roundtrip() {
        ...
        frame.set(2, 3, cell);
        assert_eq!(*frame.get(2, 3).expect("Failed to get frame cell"), cell);
    }
    ...
    fn frame_oob_set_is_noop() {
        ...
        for y in 0..4 {
            for x in 0..4 {
                assert_eq!(
                    *frame.get(x, y).expect("Failed to get frame cell"),
                    CogmindCell::default()
                );
            }
        }
    ...
    fn widget_renders_cell_colors() {
        ...
        let c00 = buf.cell((0, 0)).expect("Failed to get cell");
        assert_eq!(c00.symbol(), "@");
        assert_eq!(c00.fg, Color::Rgb(255, 0, 0));
        assert_eq!(c00.bg, Color::Rgb(0, 255, 0));

        let c10 = buf.cell((1, 0)).expect("Failed to get cell");
    ...
    fn widget_blank_cells_are_spaces() {
        ...
        for y in 0..3u16 {
            for x in 0..3u16 {
                let cell = buf.cell((x, y)).expect("Failed to get cell");
    ...
    fn widget_draws_out_of_bounds_clipped_gracefully() {
        ...
        let cell = buf.cell((5, 5)).expect("Failed to get cell");
        assert_eq!(cell.symbol(), " ");
    }
