...
pub fn encode_doom_sixel(
    ...
    write!(out, "\x1bP7;0;{}q", area_w).expect("write! to String should never fail");
    ...
    for (i, is_used) in used.iter().enumerate() {
        ...
        write!(out, "#{};2;{};{};{}", i, r, g, b).expect("write! to String should never fail");
    }
    ...
    for band in 0..bands {
        ...
        for &color in &dirty_colors {
            ...
            write!(out, "#{}", color).expect("write! to String should never fail");
...
fn emit_rle(out: &mut String, ch: u8, count: usize) {
    if count >= 4 {
        write!(out, "!{}{}", count, ch as char).expect("write! to String should never fail");
    } else {
        for _ in 0..count {
            out.push(ch as char);
        }
