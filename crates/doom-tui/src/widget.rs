    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        match (self.scaling_mode, self.char_set) {
            (ScalingMode::Nearest, None) => {
                // Inline coordinate arithmetic saves multiple Vec allocations per frame.
                for cy in 0..term_h {
                    let y_top = (cy * fb_h) / term_h;
                    let y_bot = (((cy * 2 + 1) * fb_h) / (term_h * 2)).min(fb_h - 1);
                    // Row offsets computed once per outer iteration (eliminates fb_w mul in inner loop).
                    let top_row_base = y_top * fb_w;
                    let bot_row_base = y_bot * fb_w;
...
mod tests {
    ...
    fn scaling_nearest_neighbor_fills_all_cells() {
        ...
        for cy in 0..10u16 {
            for cx in 0..20u16 {
                let cell = buf.cell((cx, cy)).expect("value must exist in test");
