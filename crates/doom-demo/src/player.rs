...
mod tests {
    ...
    fn from_lmp_parses_valid_file() {
        ...
        let player = player.expect("valid LMP parsing should return Some");
    ...
    fn next_tic_cmds_returns_in_order() {
        ...
        let lmp = rec.to_lmp();
        let mut player = DemoPlayer::from_lmp(&lmp).expect("valid LMP parsing should return Some");

        let t0 = player.next_tic_cmds().expect("tic commands should exist");
        assert_eq!(t0[0].forward_move, 0);

        let t1 = player.next_tic_cmds().expect("tic commands should exist");
        assert_eq!(t1[0].forward_move, 10);

        let t2 = player.next_tic_cmds().expect("tic commands should exist");
    ...
    fn peek_tic_does_not_advance() {
        let lmp = record_n_tics(3);
        let player = DemoPlayer::from_lmp(&lmp).expect("valid LMP parsing should return Some");
    ...
    fn next_tic_returns_ticcmd() {
        ...
        let cmd: TicCmd = player
            .next_tic()
            .expect("next_tic should return a valid command");
