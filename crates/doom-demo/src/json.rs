...
//!
//! This module provides the `export_demo_to_json` function, which translates
//! the parsed demo tics and player input commands into a human-readable JSON array.
...
pub fn export_demo_to_json(player: &mut DemoPlayer) -> String {
    ...
    while let Some(cmds) = player.next_tic_cmds() {
        for (player_idx, cmd) in cmds.iter().enumerate() {
            ...
            let _ = write!(
                out,
                r#"  {{"tic":{},"player":{},"forward_move":{},"side_move":{},"angle_turn":{},"buttons":{}}}"#,
...
mod tests {
    ...
    fn test_export_demo_to_json() {
        ...
        assert!(json.contains(
            r#"{"tic":0,"player":0,"forward_move":50,"side_move":-10,"angle_turn":3,"buttons":31}"#
        ));
