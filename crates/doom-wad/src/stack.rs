...
impl WadStack {
    ...
    /// use doom_wad::WadStack;
    /// let mut stack = WadStack::new();
    /// stack.push_iwad(b"IWAD\0\0\0\0\x0C\0\0\0".to_vec()).expect("must succeed");
    ///
...
mod tests {
    ...
    fn pwad_overrides_iwad_lump() {
        ...
        // PWAD wins for "DEMO"
        assert_eq!(
            stack.lump_data("DEMO").expect("value must exist in test"),
            b"pwad_data"
        );
        // IWAD lump not in PWAD is still accessible
        assert_eq!(
            stack.lump_data("UNIQUE").expect("value must exist in test"),
            b"only_in_iwad"
        );
    }
    ...
    fn pwad_shadowing_preserves_iwad_lumps() {
        ...
        // But B is overridden
        assert_eq!(
            stack.lump_data("B").expect("value must exist in test"),
            b"b_override"
        );
    }
