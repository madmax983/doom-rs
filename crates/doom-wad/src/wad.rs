...
/// let wad = WadFile::parse(wad_bytes).expect("wad parse or lookup must succeed");
///
/// assert_eq!(wad.lump_count(), 1);
/// assert_eq!(wad.find_lump_data("TEST").expect("wad parse or lookup must succeed"), b"DATA");
/// ```
#[derive(Debug)]
pub struct WadFile {
    kind: WadKind,
...
impl WadFile {
    ...
    /// let wad = WadFile::parse(wad_bytes).expect("wad parse or lookup must succeed");
    ///
    /// let lump = wad.find_lump("TEST").expect("wad parse or lookup must succeed");
    ...
    /// let wad = WadFile::parse(wad_bytes).expect("wad parse or lookup must succeed");
    ///
    /// assert_eq!(wad.find_lump_data("test").expect("wad parse or lookup must succeed"), b"DATA"); // Search is case-insensitive
