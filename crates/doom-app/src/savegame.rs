...
pub fn apply_save(gs: &mut GameState, payload: &SavePayload) -> Result<(), SaveError> {
    ...
    // Restore statistics.
    gs.level_stats.kill_count = payload.kill_count;
    gs.level_stats.item_count = payload.item_count;
    gs.level_stats.secret_count = payload.secret_count;
...
fn build_payload(gs: &GameState) -> SavePayload {
    ...
    SavePayload {
        ...
        kill_count: gs.level_stats.kill_count,
        item_count: gs.level_stats.item_count,
        secret_count: gs.level_stats.secret_count,
    }
}
