...
/// # use doom_tui::DoomApp;
/// # let doom_game: DoomGame = unsafe { std::mem::zeroed() };
/// let demo_bytes = std::fs::read("my_demo.lmp").expect("file read or parse must succeed");
/// let player = DemoPlayer::parse(&demo_bytes).expect("file read or parse must succeed");
