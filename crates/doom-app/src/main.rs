...
struct Args {
    ...
    /// Run sight analysis to find overwatch points and blind spots. (Requires 'analyze_sight' feature)
...
fn run_doom(args: Args, overrides: CliOverrides) -> Result<()> {
    ...
    #[cfg(feature = "analyze_sight")]
    if args.analyze_sight {
        let analyzer = doom_map::SightAnalyzer::new(&level.reject);
        let overwatch = analyzer.overwatch_sectors(5);
        let blind = analyzer.blind_spots(5);

        if args.json {
            let mut overwatch_json = String::from("[");
            for (i, (sector, score)) in overwatch.iter().enumerate() {
                if i > 0 {
                    overwatch_json.push_str(", ");
                }
                overwatch_json.push_str(&format!(
                    r#"{{"sector": {}, "visible_sectors": {}}}"#,
                    sector, score
                ));
            }
            overwatch_json.push(']');

            let mut blind_json = String::from("[");
            for (i, (sector, score)) in blind.iter().enumerate() {
                if i > 0 {
                    blind_json.push_str(", ");
                }
                blind_json.push_str(&format!(
                    r#"{{"sector": {}, "visible_sectors": {}}}"#,
                    sector, score
                ));
            }
  ...
  "blind_spots": {}
                ...
                warp_str, overwatch_json, blind_json
            );
            println!("{json_data}");
        } else {
            use crossterm::style::Stylize;
            ...
            if is_tty {
                println!("{} Sight Analysis for {}:", "👁️ ".cyan(), warp_str.bold());
                println!("  {} Overwatch Points:", "🎯".green());
                for (sector, score) in overwatch {
                    println!(
                        "    Sector {}: {} visible sectors",
                        sector.to_string().yellow(),
                        score.to_string().cyan()
                    );
                }
                println!("  {} Blind Spots:", "🦇".dark_grey());
                for (sector, score) in blind {
                    println!(
                        "    Sector {}: {} visible sectors",
                        sector.to_string().dark_grey(),
                        score.to_string().cyan()
                    );
                }
            } else {
                println!("Sight Analysis for {}:", warp_str);
                println!("  Overwatch Points:");
                for (sector, score) in overwatch {
                    println!("    Sector {}: {} visible sectors", sector, score);
                }
                println!("  Blind Spots:");
                for (sector, score) in blind {
                    println!("    Sector {}: {} visible sectors", sector, score);
