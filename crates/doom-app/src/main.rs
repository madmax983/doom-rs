...
fn run_verify_demo(args: &Args, wad_stack: &WadStack, source: &str) -> Result<()> {
    ...
    if is_tty {
        table.set_header(vec![
            comfy_table::Cell::new("doom-rs demo verification")
    ...
    let add_row = |table: &mut comfy_table::Table, key: &str, value: String| {
        if is_tty {
            table.add_row(vec![
                comfy_table::Cell::new(key),
                comfy_table::Cell::new(value).fg(comfy_table::Color::Yellow),
            ]);
        } else {
            table.add_row(vec![
                comfy_table::Cell::new(key),
    ...
    add_row(&mut table, "🎥 Demo Source", source_label.to_string());
    ...
    if header.nomonsters || header.fast || header.respawn {
        if is_tty {
            table.add_row(vec![
                comfy_table::Cell::new("⚠️  WARNING").fg(comfy_table::Color::Red).add_attribute(comfy_table::Attribute::Bold),
                comfy_table::Cell::new("nomonsters/fast/respawn are NOT wired into spawn_level_things; this replay ignores them and may desync from vanilla.").fg(comfy_table::Color::Red),
            ]);
        } else {
            table.add_row(vec![
                comfy_table::Cell::new("WARNING"),
                comfy_table::Cell::new("nomonsters/fast/respawn are NOT wired into spawn_level_things; this replay ignores them and may desync from vanilla."),
            ]);
        }
    }
    ...
    let demo_tics = { DemoPlayer::from_lmp(&demo_bytes).map_or(0, |p| p.total_tics()) };
    add_row(&mut table, "⏱️  Demo Tics (File)", demo_tics.to_string());
    add_row(
        &mut table,
        "⏱️  Total Tics Played",
        r0.total_tics.to_string(),
    );
    add_row(&mut table, "🛑 Stop Reason", r0.reason.as_str().to_string());
    ...
    add_row(&mut table, "📍 Final Player Pos", pos_str);
    add_row(
        &mut table,
        "🧭 Final Angle (BAM)",
        r0.final_angle.to_string(),
    );
    add_row(&mut table, "❤️  Final Health", r0.final_health.to_string());

    let stats_str = format!(
        "kills={} items={} secrets={}",
        r0.final_kills, r0.final_items, r0.final_secrets
    );
    add_row(&mut table, "📊 Final K/I/S", stats_str);
    add_row(
        &mut table,
        "🎲 Final RND Index",
        r0.final_rndindex.to_string(),
    );

    let (det_text, det_color) = if determinism_pass {
        ("PASS".to_string(), comfy_table::Color::Green)
    } else {
        (
            format!(
    ...
    if let Some(ref log_path) = args.verify_log {
        add_row(&mut table, "📝 Per-Tic Log", log_path.display().to_string());
    }
