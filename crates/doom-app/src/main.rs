...
impl DoomGame {
    fn tick_title_screen(&mut self, input: &TicInput) {
    ...
    fn tick_phase_controller(&mut self, transition_pressed: bool) -> bool {
        match self.phase_controller.phase() {
...
impl DoomApp for DoomGame {
    fn tick(&mut self, input: TicInput) {
        ...
        if self.title_screen.is_some() {
            self.tick_title_screen(&input);
            return;
        }

        if self.tick_phase_controller(transition_pressed) {
            return;
        }
