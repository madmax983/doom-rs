...
fn run_verify_demo(args: &Args, wad_stack: &WadStack, source: &str) -> Result<()> {
    ...
    if is_tty {
        ...
        if header.nomonsters || header.fast || header.respawn {
            table.add_row(vec![
                comfy_table::Cell::new("⚠️  Warning").fg(comfy_table::Color::Red),
                comfy_table::Cell::new("nomonsters/fast/respawn are NOT wired into spawn_level_things; replay ignores them and may desync.").fg(comfy_table::Color::Red),
            ]);
        }

        table
            .add_row(vec![
                comfy_table::Cell::new("⏳ Demo Tics"),
                comfy_table::Cell::new(demo_tics.to_string()),
            ])
            .add_row(vec![
