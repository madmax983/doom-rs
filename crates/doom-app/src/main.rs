...
fn run_verify_demo(args: &Args, wad_stack: &WadStack, source: &str) -> Result<()> {
    ...
    if is_tty {
        println!(
            "{} {}",
            "✨".cyan(),
            "doom-rs demo verification".cyan().bold(),
        );
    } else {
    ...
    let diff_run_str = diff_run.unwrap_or(0);
    let diff_row_str = first_diff_row
        .map(|r| r.to_string())
        .unwrap_or_else(|| "unknown".to_owned());

    if is_tty {
        table.set_header(vec![
            comfy_table::Cell::new("Attribute")
        ...
        table.add_row(vec![
            comfy_table::Cell::new("⏱️  Tics in File"),
            comfy_table::Cell::new(parsed_tics.to_string()).fg(comfy_table::Color::Cyan),
        ]);
        table.add_row(vec![
        ...
        table.add_row(vec![
            comfy_table::Cell::new("🎲 Final RNG Idx"),
            comfy_table::Cell::new(r0.final_rndindex.to_string()),
        ]);

        let (det_text, det_color) = if determinism_pass {
            (format!("PASS ({runs}x)"), comfy_table::Color::Green)
        } else {
            (
                format!("FAIL (run {diff_run_str} differs; first diff row {diff_row_str})"),
                comfy_table::Color::Red,
            )
        };
        table.add_row(vec![
        ...
        table.add_row(vec![
            comfy_table::Cell::new("Tics in File"),
            comfy_table::Cell::new(parsed_tics.to_string()),
        ]);
        table.add_row(vec![
        ...
        let det_text = if determinism_pass {
            format!("PASS ({runs}x)")
        } else {
            format!("FAIL (run {diff_run_str} differs; first diff row {diff_row_str})")
        };
