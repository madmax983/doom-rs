...
fn run_verify_demo(args: &Args, wad_stack: &WadStack, source: &str) -> Result<()> {
    ...
    if is_tty {
        ...
        let file_tics = DemoPlayer::from_lmp(&demo_bytes).map_or(0, |p| p.total_tics());
        table.add_row(vec![
            comfy_table::Cell::new("⏱️ File Tics"),
            comfy_table::Cell::new(file_tics.to_string()),
        ]);
        table.add_row(vec![
        ...
        if determinism_pass {
            table.add_row(vec![
                comfy_table::Cell::new(format!("⚖️ Determinism ({}x)", runs)),
