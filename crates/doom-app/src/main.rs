...
fn run_verify_demo(args: &Args, wad_stack: &WadStack, source: &str) -> Result<()> {
    ...
    if is_tty {
        ...
        if header.nomonsters || header.fast || header.respawn {
            table.add_row(vec![comfy_table::Cell::new("⚠️  Warning"), comfy_table::Cell::new("nomonsters/fast/respawn are NOT wired into spawn_level_things; this replay ignores them and may desync from vanilla.").fg(comfy_table::Color::Red)]);
        }
        let _ = flags_set;
        table.add_row(vec![
            comfy_table::Cell::new("⏱️  Demo Tics"),
            comfy_table::Cell::new(demo_tics.to_string()).fg(comfy_table::Color::Green),
        ]);
        table.add_row(vec![
            comfy_table::Cell::new("▶️  Tics Played"),
            comfy_table::Cell::new(r0.total_tics.to_string()).fg(comfy_table::Color::Green),
        ]);
        table.add_row(vec![
        ...
        table.add_row(vec![
            ...
            comfy_table::Cell::new(r0.final_health.to_string()).fg(comfy_table::Color::Red),
        ]);
        table.add_row(vec![
        ...
        table.add_row(vec![
            comfy_table::Cell::new("🎲 Final RND"),
            comfy_table::Cell::new(r0.final_rndindex.to_string()),
        ]);
        if determinism_pass {
            table.add_row(vec![
                comfy_table::Cell::new("🔬 Determinism"),
                comfy_table::Cell::new(format!("PASS ({}x)", runs))
        ...
        println!("header flags      : {flags_str}");
        if header.nomonsters || header.fast || header.respawn {
            println!(
        ...
        let _ = flags_set;
        println!("demo tics in file : {demo_tics}");
