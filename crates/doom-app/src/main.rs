...
fn run_verify_demo(args: &Args, wad_stack: &WadStack, source: &str) -> Result<()> {
    ...
    if is_tty {
        ...
        table.add_row(vec![
            comfy_table::Cell::new("demo source"),
            comfy_table::Cell::new(source_label.clone()).fg(comfy_table::Color::Yellow),
        ]);
        table.add_row(vec![
            comfy_table::Cell::new("header"),
            comfy_table::Cell::new(format!(
                "version={} skill={} episode={} map={} -> warp={}",
                header.version, header.skill, header.episode, header.map, warp_str
            ))
            .fg(comfy_table::Color::Yellow),
        ]);
        table.add_row(vec![
            comfy_table::Cell::new("header flags"),
            comfy_table::Cell::new(format!(
        ...
        if header.nomonsters || header.fast || header.respawn {
            table.add_row(vec![
                comfy_table::Cell::new("WARNING").fg(comfy_table::Color::Red).add_attribute(comfy_table::Attribute::Bold),
                comfy_table::Cell::new("nomonsters/fast/respawn are NOT wired into spawn_level_things;\nthis replay ignores them and may desync from vanilla.").fg(comfy_table::Color::Red),
            ]);
        }
        table.add_row(vec![
            comfy_table::Cell::new("demo tics in file"),
            comfy_table::Cell::new(
                DemoPlayer::from_lmp(&demo_bytes)
        ...
        table.add_row(vec![
            comfy_table::Cell::new("total tics played"),
            comfy_table::Cell::new(r0.total_tics.to_string()).fg(comfy_table::Color::Yellow),
        ]);
        table.add_row(vec![
            comfy_table::Cell::new("stop reason"),
            comfy_table::Cell::new(r0.reason.as_str()).fg(comfy_table::Color::Yellow),
        ]);
        table.add_row(vec![
            comfy_table::Cell::new("final player pos"),
            comfy_table::Cell::new(format!(
                "raw=({},{},{})  int=({},{},{}) map units",
                r0.final_px_raw,
                r0.final_py_raw,
        ...
        table.add_row(vec![
            comfy_table::Cell::new("final angle (BAM)"),
            comfy_table::Cell::new(r0.final_angle.to_string()).fg(comfy_table::Color::Yellow),
        ]);
        table.add_row(vec![
            comfy_table::Cell::new("final health"),
            comfy_table::Cell::new(r0.final_health.to_string()).fg(comfy_table::Color::Green),
        ]);
        table.add_row(vec![
        ...
        table.add_row(vec![
            comfy_table::Cell::new("final rndindex"),
            comfy_table::Cell::new(r0.final_rndindex.to_string()).fg(comfy_table::Color::Yellow),
        ]);
        if determinism_pass {
            table.add_row(vec![
